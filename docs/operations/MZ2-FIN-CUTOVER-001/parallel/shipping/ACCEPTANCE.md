...
وجود `expected_result=403` أو `needs_review` أو قائمة قيود فارغة لا يثبت تنفيذ رفض صلاحية أو منع تكرار أو إقفال أو إيقاف. الفاحص يراجع اتساق توقعات التصميم فقط. S10/S11 لهما فحص حسابي نقي للحدود، لا استدعاء لخدمة P02. قيد S30 الموسوم `external_consumer_expectation` و`consumer=P03` نتيجة متوقعة لمستهلك خارجي للعقد، وليس قيدًا ستنشئه وحدة الشحن.
...
## تصحيح مراجعة المشرف R1–R4

بدأ التصحيح من HEAD المحلي والبعيد `7ee877a0b19e12b997d2f7e5fe484b0ede9b86ff`، مع worktree نظيف؛ لم يوجد تصحيح سابق. التصحيح على فرع وDraft #1108 نفسيهما. لم تعاد قراءة ملفات التطبيق، ولم تعدل ملفات المخزون أو النواة أو STATUS أو الإصدار.

فُصل `validate_data(data)` الحسابي عن فحص Git/الملفات. يشغّل `test-prep.py` نسخًا متعمدة الخطأ في الذاكرة ويطلب كود رفض محددًا؛ لا يُعد أي exception عشوائي نجاحًا، ولا تتغير fixtures الأصلية أثناء الاختبارات.

| نوع الفحص الفعلي | النتيجة |
|---|---|
| الحسابات والأرصدة | 33 حالة أصلية ناجحة؛ 30 قيدًا متوازنًا متوقعًا، منها قيد واحد لتوقع مستهلك P03 الخارجي |
| مبالغ البنك/COD/المقاصة | 3 أمثلة S14–S16 مربوطة بـsettlement_event_key وطرف واضح؛ وصول البنك وأرجل COD/payable والمعادلة يفحصها نوع net_inflow/explicit_offset فقط |
| صافي صفر S15 | وصول0، بلا receipt/reference بنك وبلا أرجل بنك؛ statement_ref والطرف إلزاميان |
| النقل والتخصيص S30 | أصل واحد net30/VAT4.50/payable34.50؛ inbound20 وoutbound10 يطابقان source legs وcost_origin_key؛ استهلاك P03 محدود بالتخصيص وبلا ذمة/بنك/ضريبة ثانية |
| الشرائح | 5 أمثلة رسوم/VAT مرتبطة بأرجل القيد؛ مبلغا الفجوة كلاهما خارج الشرائح؛ حساب التداخل للحد المشترك |
| النسخ السلبية | **27 نسخة متناقضة رُفضت**، كل منها بكود الرفض المطلوب |
| ضوابط صحيحة إضافية | **5 نسخ صحيحة نجحت**: حد يمين غير مشمول، حد يسار غير مشمول، حدان منفصلان، استهلاك P03 جزئي10 من20، وحركة بنك مستقلة مستبعدة من معادلة S14 |

الأوامر الفعلية من جذر worktree باستخدام Python المرفق في Codex، كلاهما exit0:

```text
python -B docs/operations/MZ2-FIN-CUTOVER-001/parallel/shipping/verify-prep.py
python -B docs/operations/MZ2-FIN-CUTOVER-001/parallel/shipping/test-prep.py
```

دليل رفض أمثلة المشرف مباشرة من المخرجات:

```text
REJECT S14_received_900: settlement_bank_legs
REJECT S15_fake_receipt: zero_offset_bank_evidence
REJECT S30_inbound_200: transport_allocation_total
REJECT S10_covered_1000: tier_gap_covered
REJECT S11_nonoverlap_rejected: tier_overlap_expectation
```

النسخ السلبية الأخرى تغطي gross/offset/party/journal/reference المخالف، وصولًا/رجل بنك في صفر المقاصة، cost origin أو net/VAT المخالف، تكرار payable/VAT أو بنك لدى المستهلك، نسب استهلاك زائدة، وحدًا متداخلًا موسومًا مقبولًا. وتشمل تبديل fee/VAT مع إبقاء القيد **متوازنًا وتحديث الأرصدة المتوقعة**، وتقسيم21/9 بدلاً من20/10 مع بقاء الإجمالي30؛ لذلك لم يعد اتزان القيد وحده كافيًا لتمرير تناقض العقد.

حدود النتيجة لم تتغير: **لم تنفذ اختبارات التطبيق أو HTTP أو Mongo أو الواجهة أو الصلاحيات أو منع التكرار التشغيلي أو الإقفال أو replay أو تعطيل القديم**. القرارات التشغيلية/الضريبية والعقود المعلقة باقية، وP02/P03 مقفلتان. لا تغيير بيانات أو Preview أو دمج أو نشر أو ترحيل.

...
## نقطة التحقق SUP-20260922-29 — مراجعة دليل نجاح 88/88 على الشجرة المحلية المجمعة

التاريخ: 2026-09-22. قُرئ سجل التسليم حتى SUP-28 ثم أحدث بيانات PRs. هذه النقطة تعتمد دليل التشغيل المحلي المرسل، ولا تدعي أن المشرف أعاد تنفيذ اختبارات التطبيق أو دخل طرفية Emergent.

### مصدر الدليل وهوية البايتات

- المرفق الكامل: `نص واحد ملصق (20260922-204244).txt`، file ID `file_00000000a524820ab77965141b79c650`.
- حسب المشرف مباشرة من بايتات المرفق: الحجم `18345` بايت؛ SHA-256 `ede0fc5f425074a9a8234eb6451f542f9f3639fd32259cb1b58fdff38d391b62`.
- جرى تحليل السجل وعدّ 83 نتيجة unittest مسماة بـ`ok`، مع ملخص pytest مستقل `5 passed`: المجموع 88، لا إضافة تشغيلات الـ45 السابقة مرة ثانية إلى العدد.
- المشغّل المبلغ عنه: `P02-1130-RESUME-EXACT-88.sh` عند commit `e588fcc7770a408f4070f1f478d85304639f62de`، blob `65920c66d9b4f6f727626f289837f6d32911409a`، SHA-256 `0a94fc9c9c7aef5333fb85325aa4453846e8ea93da889ac2464741fd9e3a7ab0`، الحجم 8726 بايت.
- الشجرة المختبرة: HEAD `20400fffb03594af8a38d6b4750c170233bd5f37` + ملفات CONTRACT_SLICE الثلاثة + Patch V4 ذي SHA-256 `14156ef2a45ae489df1d4afcda4c65f4b0804a737837ffbe93cc86f5e7b21d4a`، في `/tmp/mz2-p02-stage1.ZMgPW8/worktree`.
- هذه تغييرات working tree غير committed؛ النجاح ليس اختبارًا لرأس PR #1130 البعيد وحده.
- بيئة التشغيل في السجل: CPython `3.13.15`، الدليل المؤقت `/tmp/mz2-p02-resume88.aZUfpd`.

### نتائج التشغيل كما روجعت من السجل الخام

| المجموعة | العدد | الزمن المبلغ عنه | النتيجة |
| --- | --- | --- | --- |
| V4 contract isolation | 32 | 0.034s | OK / exit 0 |
| V4 payment evidence | 13 | 0.130s | OK / exit 0 |
| calculator regression | 13 | 0.085s | OK / exit 0 |
| contract regression | 25 | 0.006s | OK / exit 0 |
| legacy COD tiers | 5 | 0.27s | 5 passed / exit 0 |
| **المجموع** | **88** | تشغيل محلي واحد للمجموعات الخمس | **PASS للنطاق المعزول** |

السجل يثبت خروج المشغّل بـ`P02_RESUME_88_RC=0` و`RESULT=EXACT_COMBINED_88_OF_88_PASS_ONLY`، مع `COMMIT=NONE` و`PUSH=NONE` و`P02=LOCKED`.

### تحقق السلامة وحدود ما يمكن استنتاجه

- تظهر الملفات الـ11 جميعًا `OK` قبل الاختبارات، و`FILES_UNCHANGED=PASS` بعد الاختبارات.
- `STATUS_UNCHANGED=PASS`, `INDEX_SEMANTIC_UNCHANGED=PASS`, `APP_UNCHANGED=PASS`, `DIFF_CHECK=PASS` في الملخص المرسل.
- semantic index manifest المطبوع: `b12f568823218d2c3cad41bb891318a5adcc28bb33430eea144c5c74bc3732ef`.
- raw index SHA قبل/بعد متطابق هذه المرة: `eb17902b4b46c779d86218e7a597a9bd4190ce5cfba1de1bbfb47901669b0beb`.
- هذه مراجعة مستقلة لاتساق سجل نفّذه المستخدم، وليست جلسة تنفيذ مستقل أخرى على target. لم يتحقق المشرف حيًا من بقاء `/tmp` بعد رفع المرفق.
- `APP_UNCHANGED` في هذا المشغّل يعتمد على مقارنة `git status`؛ ليس snapshot شاملًا لبايتات `/app` أو قاعدة البيانات أو Preview/Production. وبصمات المصدر المراجعة هنا تخص الملفات الـ11، لا جميع تبعيات المصدر والحزم المتعدية.
- لا تُعد نتائج رفض HTTPException 423 داخل unit tests إثباتًا لاختبار HTTP حي، ولا تمثيل Decimal في الذاكرة إثباتًا لدقة حفظ الأستاذ.
- التكامل مع خدمة الأدلة الإنتاجية يبقى `NOT_INTEGRATED`، وربط الأستاذ والتقارير `NOT_CONNECTED`. لا قبول لتفعيل المسار أو أول بيع COD خارجي أو تكامل Real Mongo/HTTP/واجهة/UAT.

### أحدث رؤوس GitHub المتحققة قبل الكتابة

- #1130: open/Draft/unmerged؛ Base `5292a87a476a140ae8c3c78e88dfba7d8c83f035`؛ Head `20400fffb03594af8a38d6b4750c170233bd5f37`، ولم تُرفع ملفات V4 وCONTRACT_SLICE الجديدة إلى رأس PR وفق هذه القراءة.
- #1131: open/Draft/unmerged عند `29e4cd940b195df0164fe7bc74b07c77a0a02bfc`؛ لا نقل قبول جديد ولا فتح implementation gate.
- #1132: open/Draft/unmerged عند `405883c34f81c5d17625083b11e3c608662023af`؛ Base المعلن في PR `ab3ef10c5aedfe5e2190bc57bb8ed26afc1347f2`؛ لا إعادة قبول R1–R5 أو سياق Base الحالي في هذه النقطة.
- #1133 قبل الكتابة: open/Draft/unmerged عند `2bd670e4b1e4da245f0f7ff1bfb83116d62c1469`؛ blob السجل `5b4ccb6335a21cfa5837bd0ea88e023a16f70ee9`.

### الحكم والخطوة الآمنة التالية

`LOCAL_COMBINED_88_TEST_EVIDENCE_ACCEPTED / READY_FOR_CONTROLLED_SOURCE_CHECKPOINT_REVIEW`

انتهت بوابة الاختبارات المحلية المطلوبة لهذه النسخة. لا إعادة لاختبارات الـ88 أو لاسترجاع الشجرة بلا سبب جديد. المطلوب التالي حفظ نسخة المصدر المجمعة بدل الاستمرار بالاعتماد على `/tmp`، لكن بوابة Commit/Push مستقلة عن الاختبارات: يُطلب تأكيد المالك على حفظ الملفات الـ11 المراجعة فقط في فرع #1130 مع بقائه Draft، دون Merge أو Deploy أو فتح مرحلة.

قبل أي Commit/Push مصرح: تحقق من هوية worktree والفرع والـHead والبصمات الـ11 وغياب staged/unmerged/unrelated changes، وقراءة origin/remote Head الفعليين وتعليمات المستودع. لا `git add .` أو force push أو reset/clean أو تطبيق #1126. عند تغير المصدر أو الفرع أو فقدان المسار يتوقف التنفيذ للمراجعة؛ لا يُحمل نجاح السجل إلى شجرة غير مطابقة. بعد الحفظ يُطابق Commit/Tree المنشوران بالبايتات المختبرة وتُراجع CI على الرأس الجديد، دون نقل العلامات القديمة إليه.

### الكتابات في هذه المراجعة

- كتابة توثيقية فقط إلى ملف التسليم نفسه: تحديث مؤشر أحدث نقطة في أول الملف من SUP-02 إلى SUP-29، وإضافة هذه النقطة مع إبقاء السجل السابق.
- تحليل محلي لبصمة المرفق والعدّ فقط؛ لم تُشغّل اختبارات التطبيق أو scripts المشروع بواسطة المشرف، ولم يُطبق Patch أو يُعدّل فرع #1130/#1131/#1132.
- توثيق commit الناتج وقراءة ما بعد الحفظ ونقطة Issue #1006 في المتابعة بعد تأكيد استجابات GitHub؛ لا يدّعي هذا النص مسبقًا اكتمال عمليات المتابعة.
- لا Merge أو Deploy، لا وصول أو تغيير Preview/Production، لا كتابة مالية أو تغيير صلاحيات أو فتح مراحل.

الحالة الملزمة: `P01=IN_PROGRESS`, `P02=LOCKED`, `P03=LOCKED`. قبول 88/88 محليًا لا يعني اكتمال P02 أو جاهزية المحاسبة للإنتاج.


---

## نقطة التحقق SUP-20260923-30 — تفويض المالك بالنشر إلى Preview واختبار أسبوع قبل Production

### طلب المالك وحدوده

طلب عرفات صراحة: «نشتي ننشره الى البرفيو عشان نكمل المحسابه بالبرفيو ونختبره لمدة اسبوع قبل النشر الى بردوكشن».

هذا تفويض جديد بالنشر إلى Preview لاستكمال المحاسبة والتجربة، ويشمل حفظ المصدر المراجع اللازم لهذا النشر في فرع المهمة؛ لا يلزم إعادة سؤال المالك عن الموافقة نفسها. لا يشمل تفويضًا بـMerge إلى فرع Production أو Redeploy/Republish للإنتاج أو تفعيل كتابات مالية حقيقية أو تجاوز حواجز النواة والأدلة. لا يُفهم مرور أسبوع كإذن آلي بالنشر إلى Production. تبقى P01 مفتوحة وP02/P03 مقفلتين حتى تحقق بوابات كل مرحلة.

### ما تحقق مباشرة في هذه المراجعة

- أُعيدت قراءة نقطة SUP-29 وتعليمات `AGENTS.md` عند `20400fffb03594af8a38d6b4750c170233bd5f37`، blob `0e0bbff4d9625f9323affae84b620a0cf12eb209`. التعليمات تمنع إبقاء العمل المهم محصورًا في /tmp، وتشترط قراءة Release Guard قبل تغيير /app أو البدء في إجراءات إصدار.
- #1130 ما زال open/Draft/unmerged: Base `5292a87a476a140ae8c3c78e88dfba7d8c83f035`، Head `20400fffb03594af8a38d6b4750c170233bd5f37`. نجاح 88/88 ما زال خاصًا بالـworking tree المجمعة في SUP-29، وليس دليل CI على مصدر جديد منشور.
- #1124 ما زال open/Draft/unmerged عند `5292a87a476a140ae8c3c78e88dfba7d8c83f035`، ويصف مساحة المحاسبة اليومية المجمعة. لا يُعاد تفسير وصفه كقبول UAT جديد أو نشر فعلي.
- #1133 قبل كتابة هذه النقطة: `72cb70998a3e2bff57b348a82751fe7296bbc217`، blob السجل `5fe7ee40d96ce51f612588389fcc11601735d929`.
- مرجع فرع أدوات Preview `codex/preview-persistent-runtime-20260919` أعاد `9975202d698e935b88942ed4196a51dfdd98e76b`. قُرئ سجل `docs/operations/PREVIEW-PERSISTENT-RUNTIME-20260919.md` وملف `scripts/preview_persistent_runtime.py`، blob الأخير `7e4b43607abfe3497eb602a4e131c03d5443fad5`.
- هذا المشغّل التاريخي يثبت مصدرًا أقدم `e82918d130f42b228346b2cbd3d70ff4daf21bf3` ومسارات دائمة تحت `/app/.worktrees/preview-runtime-state-20260919` وحجزًا خاصًا. كما يحوي رابطًا لملف `/app/backend/.env`؛ لذلك لا يُعاد تشغيل prepare/activate القديم أو تبديل SHA وحده أو افتراض عزل قاعدة البيانات دون مراجعة الإعداد الفعلي.
- سجل Preview التاريخي يذكر منع الشبكة الخارجية والعمال الآليين، واستمرار التشغيل بعد restart، وقاعدة Mongo محلية standalone آنذاك. هذه معلومات تاريخية وليست مشاهدة حية للطوبولوجيا أو الحجز اليوم. يلزم إثبات قاعدة اختبار معزولة تدعم المعاملات قبل اختبارات الترحيل المتكاملة.
- حاولت أداة الويب قراءة `/preview-meta.json` و`/api/preview-auth-policy` على نطاق Preview؛ أعادت عدم الإتاحة عبر الأداة. لا يُسجل ذلك كـ502 أو كدليل تعطل Preview.
- استكشاف القدرات لم يوفر اتصالًا مباشرًا بطرفية Emergent في الجلسة. لذلك لم يُجرَ Commit/Push للمصدر ولا تغيير إعداد Supervisor أو تشغيل build/activate/restart. لا توجد نتيجة نشر أو HTTP قبول حية جديدة.

### القرار التنفيذي

`PREVIEW_DEPLOYMENT_AUTHORIZED / SOURCE_CHECKPOINT_AND_RUNTIME_PREFLIGHT_PENDING`

الترتيب المعتمد للتحضير:
1. قراءة الحالة الفعلية لحارس الإصدار ونسخة المصدر وSupervisor وmetadata الخاصة بـPreview والحجز والتخزين. لا كشف أسرار أو طباعة .env أو حذف حجز عامل آخر.
2. حفظ الملفات الـ11 المطابقة للبايتات المختبرة في فرع #1130 وترك PR Draft، بعد تحقق الهوية والنطاق وتعليمات المستودع. لا `git add .` ولا force push ولا إعادة نفس الاختبارات بلا تغير؛ عند فقدان /tmp لا يُنسب قبول جديد لشجرة غير مفحوصة.
3. إعداد إصدار Preview معزول بمصدر مثبت وتخزين دائم مثبت، دون تبديل فرع `/app` الإنتاجي أو الكتابة فوق إصدار عامل آخر. الحفاظ على إصدار رجوع معروف وملفات بيانات Preview الموجودة.
4. إثبات عزل قاعدة البيانات والأدلة والجلسات عن Production، وتعطيل الاتصالات التشغيلية إلى قيود وسلة والدفع والشحن والإعلانات. لا إلغاء حراس المعاملات أو الإقفال أو الصلاحيات لتجاوز أخطاء التجربة.
5. بناء الواجهة والخلفية من المصدر المرشح نفسه، ثم تفعيل Preview وحده بعد تحقق الحجز والحارس، وإثبات هوية FE/BE واختبارات الدخول وHTTP والدورة المحاسبية الأساسية.

V4 الحالية تثبت إغلاق المسار غير المتكامل؛ لا يُفتح `require_native_contract_runtime()` بمجرد النشر. تبقى خدمة الأدلة `NOT_INTEGRATED` والأستاذ/التقارير `NOT_CONNECTED` حتى اكتمال الربط واختباراته. يبدأ الاستكمال داخل Preview بما هو عامل، مع بيان واضح للأجزاء المحجوبة؛ لا تُجمع #1126/#1131/#1132 أو P03 ضمنيًا في هذا الإصدار.

### أسبوع الاختبار قبل الإنتاج

الخطة: إتاحة نسخة Preview للتطوير والاختبار أولًا، ثم تثبيت نسخة مرشحة قابلة لتجربة الدورة المطلوبة وبدء سبعة أيام قبول مسجلة من نجاح النشر والفحوص الأولية. لا يبدأ العد من رسالة التفويض أو من اختبارات الوحدة. يغطي القبول الأرصدة الافتتاحية التجريبية والمبيعات والاستردادات والتسويات والحركات والشحن المتكامل عند جاهزيته، مع مطابقة القيود والذمم والبنوك والضريبة، ومنع التكرار والصلاحيات والفترات المقفلة واستمرار البيانات بعد restart. أي تغيير جوهري يسجل بهوية جديدة وتُعاد اختبارات الأجزاء المتأثرة. إنهاء الأسبوع بلا أخطاء حرجة شرط مراجعة، وليس نشرًا تلقائيًا للإنتاج؛ Production يحتاج موافقة مستقلة بعد التقرير النهائي.

### الكتابات والاختبارات في هذه النقطة

- تحديث ملف التسليم نفسه فقط على فرع #1133، مع الحفاظ على checkpoints السابقة وتحديث مؤشر أحدث نقطة والتاريخ.
...
## نقطة التحقق SUP-20260923-31 — تثبيت مصدر #1130 وبداية تنفيذ #1131 المعزول

التاريخ: 2026-09-23. هذه النقطة توثق تقرير المنفذ ثم تحقق المشرف مستقلًا من GitHub لما أمكن.

### ما تحقق منه المشرف مباشرة

- PR #1130 ما زال open/Draft/unmerged.
- Base بقي:
  `5292a87a476a140ae8c3c78e88dfba7d8c83f035`.
- Head البعيد أصبح:
  `37a5ab60a8c42736c63be37457ae9e7019e82415`.
- مقارنة الرأس السابق `20400fffb03594af8a38d6b4750c170233bd5f37` بالجديد:
  - ahead by 1 commit
  - behind by 0
  - التغيير يقتصر على **11 مسارًا** فقط، وهي نفس قائمة V4 + CONTRACT_SLICE التي سبق قبول دليل 88/88 عليها.
- check-run الوحيد الظاهر على الرأس الجديد:
  - `backend-unit`
  - status `completed`
  - conclusion `success`
  - job id `106947148375`
  - workflow run `35787315588`.
- PR #1131 ما زال open/Draft/unmerged ورأسه البعيد ما زال:
  `29e4cd940b195df0164fe7bc74b07c77a0a02bfc`.
  لذلك لا يوجد حتى هذه القراءة تنفيذ #1131 منشور في GitHub.

### ما بقي تقرير منفذ ولم يُتحقق منه مستقلًا هنا

- أن جميع SHA-256 للـ11 مسارًا المنشورة تطابق البايتات المختبرة محليًا؛ نطاق الملفات نفسه تحقق، لكن إعادة حساب SHA-256 للبايتات البعيدة لم تُنفذ في هذه النقطة.
- أن شجرة تنفيذ #1131 المعزولة المحلية بدأت فوق #1130 الجديد ولم تُدفع بعد.
- أن Preview وProduction لم يتغيرا ولم تحدث كتابة مالية؛ لم يدخل المشرف إلى البيئتين في هذه النقطة.

### الحكم

`PR1130_SOURCE_CHECKPOINT_PUBLISHED_SCOPE_VERIFIED / NEW_HEAD_CI_PARTIAL_PASS`

نجاح `backend-unit` على الرأس الجديد دليل CI صالح لهذا الرأس، لكنه ليس بمفرده إعادة إثبات كل 88 حالة ولا تكامل Preview/HTTP/Real Mongo/UI.

الخطوة الآمنة التالية:
1. يكمل المنفذ #1131 في الشجرة المعزولة فوق #1130 الجديد، دون Push ناقص.
2. قبل أول Push لـ#1131، يعرض diff/scope والاختبارات التي نجحت فعليًا، ويفصل backend/frontend/permissions/evidence/V2-transition.
3. لا يبدأ Preview Candidate أو أسبوع UAT قبل اكتمال #1131 المطلوب وبناء مرشح مصدر مضبوط.

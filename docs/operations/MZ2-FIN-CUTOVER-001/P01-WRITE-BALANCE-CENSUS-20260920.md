...
Frozen accepted report source: `163874f5ffcab10a5f3dc883a1c2acd56c414bf9` (#1110 A). Its A/B and evidence remain preserved. This **final integration census** was regenerated from `codex/p01-write-balance-release-20260920`, based on freshly fetched Production J `6365a042dfcb125e81e5e198ea1ff1537373ce51`, after integrating the accepted source and write-balance WIP checkpoint `696b4`. It includes documented cross-provider execution, canonical provider-identity validation and updated fixtures. The CSV records SHA-256 of the actual indexed source-file bytes. A/B will be assigned only after this source is frozen; any subsequent source edit requires checking the affected index/hash again.
...
## Production baseline delta and preservation

Final J: `6365a042dfcb125e81e5e198ea1ff1537373ce51` (merge of customer-cohort clarity PR #1112), replacing the earlier observed `80d84ffc599dc373dbb4a2bb45e226fa07ffde4a`. Read-only `git diff --name-only ebf92e6606a810a1e3f0a0e2026984a5482f71bc 6365a042dfcb125e81e5e198ea1ff1537373ce51` lists 13 changed files: customer-cohort backend report/route/tests; frontend report/service/navigation/tests; `.gitignore`; that task's status; and release intent. The newer `80d84..6365a` delta changes the cohort frontend report/test, its task status and intent.

Verified preservation after source integration: `git diff --name-only 6365a042dfcb125e81e5e198ea1ff1537373ce51 --` followed by all 12 non-intent files in that delta returned **empty**. `git ls-files -u` also returned empty. Thus the cohort source, clarity change, tests, navigation, `.gitignore` and its status remain identical to final J; no overlapping P01 writer/report change or unresolved conflict was found. Old #1110 B intent is not transferred as a source change. These are local source checks, not Production actions.

## Inspected P01 source byte hashes

The six original balance calls are absent from these P01 source files. Hashes below cover the files that a negative symbol search alone would not list.

| File | SHA-256 |
|---|---|
| `backend/accounting_customer_refunds.py` | `ea30620277ec8b2b6182273408eacf3ea66f517ac5a028a32a9855f5986c43c0` |
| `backend/accounting_customer_advances.py` | `7af04c6d74ba353df7e242ecaa8940c3a643b696a3684320946b974d0348d729` |
| `backend/accounting_settlement_service.py` | `8cacaa51c450ff88213c6d41f7039f7052b2932084f2f3a287238f5e8c636586` |
| `backend/accounting_mz2_balances.py` | `0545b02e281241e037ac8e5bddba91e5991283da86a8694959efaf1e6695be97` |
| `backend/accounting_mz2_reports.py` | `804aeae8e39a5891d6f99bfd5115a243868bc192419be10722969bcf119dcf7b` |
...
Counts for final integration snapshot: MZ2_shipping_P02_outside_P01=3 matched lines, legacy_unrelated=524 matched lines, test_only=242 matched lines; production-source files=66; total CSV records=769.

...
The design Preview may skip MFA and email OTP only when all three runtime
guards are present:
...
`AUTH_PREVIEW_PASSWORD_ONLY`. Even if one setting is copied accidentally, the
remaining environment and exact-host guards keep the bypass closed.

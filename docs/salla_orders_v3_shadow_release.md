...
The frozen candidate is PR #1053 on `codex/salla-v3-release-current-20260916`.
The manual-sample follow-up uses `codex/salla-v3-probe-release-20260916`; its
PR and exact checkpoint are recorded in Issue #1006. Neither enables Shadow, creates a
...
- `salla_orders_v3_parity_runs` (internally created, owner-bound, one-hour
  trusted report context; the manual probe can create one internally)
- `salla_orders_v3_parity_evidence` (source of sealed artifacts bound
  to one run, verified runtime HEAD, owner, internal observation time, digest,
  and TTL; the follow-up can write a distinct `salla_orders_v3_order_probe`
  record, which the full-parity evidence reader always rejects)
...
4. Fresh regressions for Order Review, Fulfillment, Qoyod, Snapchat attribution,
   ...
   if the verified runtime HEAD changes. No caller can submit arbitrary
   evidence, regression booleans, a HEAD, owner, observation time or run id.
...
identity. The bounded owner-only diagnostic below does not introduce a manual
requeue, activation or operational cutover route.

## Owner-requested provider sample (follow-up to #1053)

The former runtime could validate supplied evidence but could not collect a
real same-runtime List/Details/Items sample without enabling the general
background worker. The follow-up supplies a manual, one-order diagnostic:

- `POST /api/salla/orders-v3/probes` accepts only string `store_id` and
  `order_number`. It derives the actor from the existing authenticated session
  and requires owner role or literal `is_owner=true`. Employees/admins without
  that owner status are rejected. Request-supplied identity/HEAD/evidence or
  unknown fields are rejected.
- `GET /api/salla/orders-v3/probes/{probe_id}` reloads and validates the saved
  sample for the same authenticated owner and verified runtime. Both responses
  use `Cache-Control: no-store`.
- This explicit diagnostic works with the background Shadow switch off. It
  does not activate that worker, schedule recovery or enqueue enrichment. There
  is no automatic caller. Invoking it on live data is a separate operational
  step after deploying a reviewed source/intent pair.
- It requires one connected owner integration, matching store, read scope, an
  unexpired existing access token and a locally stored internal Salla order id.
  It pins that token and checks integration revision/store and runtime identity
  before and after each provider read. It never refreshes credentials or
  follows redirects; expired/failed authentication stops the probe.
- It makes at most three GETs: reference-filtered List Light, identity-matched
  Details Light, and canonical Order Items. It uses the existing V3 gateway,
  engine, normalizer and compatibility mapper. No provider retry is permitted.
  One 90-second deadline covers the probe; one atomic owner/store cooldown
  permits at most one sample per five minutes, including failed/cancelled runs.
  Provider responses and the saved artifact are each capped at 1 MiB.
- It rereads the local products/internal id and refuses to seal evidence if
  those changed while fetching. The saved one-hour artifact contains only
  product identities, quantities, semantic options/custom fields, the typed
  owner/store/order identity, and endpoint response hashes/timestamps. It omits
  credentials, customer contact fields and raw provider/provenance payloads.
  Personalized option text and attachment URLs are part of the comparison and
  remain private owner data; never copy live artifacts into GitHub or CI logs.
- The artifact has a different record type and id prefix from full parity
  evidence. Its digest binds the payload to the internally created run,
  verified source HEAD, owner and observation time; readback rejects tampering,
  expiry and a changed runtime. It does not prove an atomic provider snapshot or
  the latest durable queue revision.
- A matching fulfillment sample still returns `parity_ready=false` and
  `cutover_allowed=false`. Qoyod payload/idempotency, attribution, five consumer
  regressions and the operational adapter remain explicit outstanding gates.
  This route cannot manufacture or accept their PASS values.

Verification uses synthetic records, an isolated HTTP transport through the
real gateway/engine, and a seventh real-Mongo case for simultaneous requests,
one-winner cooldown, sealed readback and unchanged operational collections.
Real store results remain unverified until a separately authorized runtime
sample is executed. The frozen #1053 A/B pair is preserved, not rewritten; this
new source needs its own build and intent before deployment.

...
- **Iter-149 (Feb 2026)**: Per-provider accounting cutoff dates. New `accounting_cutoffs` collection + `accounting_cutoffs.py` helper + `accounting_cutoffs_routes.py` (GET/PUT/POST /api/accounting/cutoffs). Defaults: Tabby 2026-04-27, Tamara 2026-04-25, Salla/COD/Bank Transfer 2026-04-30. Auto-seed on first read. BNPL settlements engine (totals + refunds + official aggregator) inline-filters by cutoff so any row dated before the merchant's cutoff is excluded. `POST /api/accounting/cutoffs/recompute?provider=X` flags entities with `is_pre_accounting=true` for downstream filtering. Frontend `/settings/accounting-cutoffs` page with 5 cards (per provider, editable date, save button) + recompute-all button. Sidebar nav added.

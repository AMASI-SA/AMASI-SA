...
- **Iter-147 (Feb 2026)**: Tamara settlement-attribution priority. Added `provider_settlement_id` / `provider_invoice_id` / `provider_settlement_date` / `provider_payout_date` / `settlement_source` / `effective_settlement_date` on `payment_transactions`. Priority: `provider_official` > `billing_eligible` > `estimated`. Tamara settlement-file import now propagates the official attribution. First-stamp-wins (re-import never clobbers). Audit log `tamara_attribution_log`. New endpoints: `GET/POST /api/bnpl/tamara/attribution/{status,recompute}`. Settlements engine groups by `effective_settlement_date`.

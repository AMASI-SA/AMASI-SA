...
- **Iter-148 (Feb 2026)**: Diagnostic + cleanup for duplicate ad-account topup rows. `GET /api/ad-accounts/diagnostics/duplicate-topups` lists `(counterparty, date, amount)` groups with count>1. `POST .../cleanup?dry_run=true|false` keeps oldest, reverses bank-tx + counterparty balance + liability paid_amount, deletes ledger row.

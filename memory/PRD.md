...
- **Iter-147 v3 (Feb 2026)**: Settlement file totals override computed totals. Helper `_aggregate_official_totals` reads `settlement_entries` per period and replaces `compute_settlement_for_provider` totals when a Tamara settlement file exists for the period. Response payload now includes `data_source` ("computed" | "provider_official_file") and `system_totals` (snapshot of what our DB would have computed, for UI diff). Frontend `BnplSettlements.jsx` shows "رسمي" badge per row when official-file numbers are used. This makes the weekly invoice match Tamara's statement to the cent.

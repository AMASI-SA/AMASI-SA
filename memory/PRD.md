...
- **Iter-147 v2 (Feb 2026)**: Refined Tamara attribution priority to use **capture date** as priority #2. New field `captured_at_provider` extracted from Tamara `captures[].created_at`. Final priority chain: `provider_official` > `provider_captured` > `billing_eligible` > `estimated`. Tamara settles by capture date — this matches their statement perfectly. Startup migration + daily sweep both opportunistically extract capture date from existing `raw_payload.captures[]`.

...
execution_path = Path("backend/mobile_preparation_execution_routes.py")
...
anchor = '''    @router.get("/files")\n    async def list_files(\n'''
helper = '''    async def _require_mobile_preparation_access(user_doc: dict[str, Any]) -> None:\n        access = await mobile_app_access_for_user(db, user_doc)\n        if not access.get("enabled") or "app.page.my_products" not in set(access.get("permissions") or []):\n            raise HTTPException(\n                status_code=403,\n                detail={\n                    "code": "mobile_app_page_permission_required",\n                    "permission": "app.page.my_products",\n                },\n            )\n\n'''
if anchor not in execution:
    raise SystemExit("mobile preparation list anchor not found")
execution = execution.replace(anchor, helper + anchor, 1)
execution = execution.replace(
    ...
    '''        reviewer = _require_reviewer(user)\n        await _require_mobile_preparation_access(reviewer)\n        user_id = _merchant_user_id(reviewer)\n''',
    2,
)
...
print("Preparation app employee selector and mobile permission alignment applied.")

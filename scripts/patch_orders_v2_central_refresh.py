...
The resulting production code stays entirely inside Mezan OS V2.  This script and
its bootstrap workflow are removed before merge.
"""
...
route_replacement = r'''    @router.get("/{order_number}")
...
service = regex_replace_once(
    ...
    r'''export async function refreshOrderFromSalla(orderNumber, { force = true } = {}) {
...
new_header = r'''                <div className="flex flex-col items-start gap-3 lg:items-end">
...
def test_salla_refresh_is_centralized_in_orders_v2():
    ...
    assert '"/orders/items"' not in refresh_source  # the approved shared helper owns this endpoint
...
for anchor, additions in (
    ...
    # Path lists occur twice (pull_request and push).  Insert into every missing
    # occurrence without duplicating already-added entries.
    if additions.strip().splitlines()[0].strip() not in workflow:

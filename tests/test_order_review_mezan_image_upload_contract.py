...
def test_review_customer_history_and_ai_loyalty_are_read_only():
    ...
    assert '"customer_id": str(customer_id)' in history_service
    assert '"GET",\n            "/orders"' in history_service
    assert 'MongoCustomerHistoryStore' not in history_service
    assert 'list_orders(' not in history_service

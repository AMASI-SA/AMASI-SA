    ...
    return {
        ...
        "capabilities": {
            ...
            "images.propose": _capability(
                "available" if connected else "not_connected",
                "يمكن حفظ طلبات تعديل الصور كاقتراحات محكومة."
                if connected else "يتطلب اتصال OpenAI أولًا.",
            ),
            "images.execute": _capability(
                "approval_required" if image_ready else "planned",
                "مزود الصور جاهز، لكن التنفيذ المباشر يبقى خلف الاعتماد البشري."
                if image_ready else "الاتصال قائم للتحليل؛ نموذج وسياسة تنفيذ الصور لم يكتمل تفعيلهما.",
            ),
        },
        "last_sync_at": None,
        "data_delay_minutes": None,
        "health": {
            "status": "healthy" if connected else "not_available",
            "score": 100 if connected else None,
            "checked_at": None,
            "data_quality": "good" if connected else "unavailable",
        },
        "latest_error": None,
        "ai": {
            "can": ([
                "تحليل بيانات ميزان ضمن العقود الآمنة والقراءة فقط",
                "حفظ اقتراحات تعديل صور المنتجات داخل دورة الاعتماد",
            ] if connected else []),
            "cannot": ([
...
def install_openai_integration_status_support() -> None:
    ...
    if getattr(cls, "_mezan_openai_status_installed", False):
        return

...
physical unit becomes its own card.  A selected Salla line with quantity 5
therefore produces five cards, each showing quantity 1, while retaining the
same order, product, image, specifications, and delivery total.
"""
...
def expand_preparation_unit_cards(
    ...
    for source in rows or []:
        ...
        # The active allocation engine always supplies unit_indices.  The
        # quantity fallback keeps this layer safe for older/internal builders.
        if not unit_indices:

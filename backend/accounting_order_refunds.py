...
async def refund_review_reasons(db, owner, draft):
    ...
    rows = await db.settlement_entries.find({'user_id': owner,
        'file_id': draft['source_file_id'], 'event_type': 'refund'}).to_list(10001)
    reasons = []
    for row in rows:
        link = await db.mz2_statement_refund_links.find_one({
            'user_id': owner, 'draft_id': draft['id'], 'entry_id': row['id']})
        if not link:
            reasons.append({'code': 'refund_identity_review:' + row['id'],
                'message': 'استرداد الطلب ' + str(row.get('order_number') or '') +
                           ' يحتاج ربطًا صريحًا بحركة الاسترداد الأصلية؛ رقم الطلب وحده لا يكفي'})
    return reasons


async def link_statement_refund(db, *, owner, draft_id, entry_id, refund_id, actor):
    ...
    async def commit(scoped):
        ...
        row = await scoped.settlement_entries.find_one({'user_id': owner, 'id': entry_id,
            'file_id': draft.get('source_file_id'), 'event_type': 'refund'})
        refunds = await scoped.payment_refunds.find({'user_id': owner, 'provider': draft['provider'],
            'provider_refund_id': refund_id}).to_list(2)
        if not row or len(refunds) != 1:
            raise HTTPException(409, 'unique_refund_identity_required')
        refund = refunds[0]
        proposal = await prepare(scoped, owner=owner, provider=draft['provider'],
            payment_id=refund['provider_payment_id'], refund_id=refund_id)
        event = proposal['event']
        amount = Decimal(str(row.get('actual_refund_amount') or 0)) + Decimal(str(row.get('actual_partial_refund_amount') or 0))
        if proposal['state'] != 'already_posted' or event['order_number'] != str(row.get('order_number')) or Decimal(event['amount']) != amount:
            raise HTTPException(409, 'refund_source_or_journal_conflict')
        key = digest([owner, proposal['event_key']])
        ...
        other = await scoped.mz2_statement_refund_links.find_one({'user_id':owner,'draft_id':draft_id,'entry_id':entry_id})
        if other and other['_id'] != key:
        ...
        await scoped.mz2_statement_refund_links.update_one({'_id':key}, {'$setOnInsert':dict(
            user_id=owner, draft_id=draft_id, entry_id=entry_id, refund_id=refund_id,
            provider=event['provider'], event_key=proposal['event_key'],
        ...
        return {'linked':True,'txn_group_id':proposal['txn_group_id'],'financial_write':False}

...
the driver app. Approval updates the review and collection, then projects a
store-delivery payment marker onto the assignment and order without overwriting
Salla-authoritative payment fields.
"""
...
def make_store_delivery_payment_review_router(db: Any, current_user: Callable[..., Any]) -> APIRouter:
    ...
    async def pending_reviews(
        ...
        actor = _require_accountant(user); user_id = _merchant_user_id(actor)
        query: dict[str, Any] = {"user_id": user_id, "status": "pending"}
        if method: query["payment_method"] = normalize_text(method)
        reviews = await db[DRIVER_PAYMENT_REVIEWS].find(query, {"_id": 0, "user_id": 0}).sort(
            "submitted_at", 1).to_list(length=limit)
        ...
        ).to_list(length=max(len(assignment_ids), 1)) if assignment_ids else []
        ...
        ).to_list(length=max(len(driver_ids), 1)) if driver_ids else []
        ...
        for review in reviews:
            ...
            items.append({
                **review,
                ...
                "driver_name": driver.get("name") or assignment.get("driver_name_snapshot"),
    ...
    async def review_payment(assignment_id: str, payload: ReviewPayload,
        ...
        now = _now(); approved = payload.decision == "approved"
        ...
        updated_review = await db[DRIVER_PAYMENT_REVIEWS].find_one_and_update(
            ...
            {"$set": {
                "status": final_status, "reviewed_at": now,
        ...
        await db[DRIVER_COLLECTIONS].update_one(
            ...
            {"$set": {
                "review_status": final_status, "reviewed_at": now,
                "reviewed_by": normalize_text(actor.get("id")),
                "review_note": normalize_text(payload.note),
        ...
        await db[ASSIGNMENTS].update_one({"user_id": user_id, "id": assignment_id}, {"$set": assignment_patch})

        # Preserve Salla's canonical fields. This namespaced projection is the
        # operational truth for customer service/order views until an explicit
        # Salla payment-status sync is designed and reviewed separately.
        order_patch = {
            "store_delivery_payment_status": assignment_patch["payment_status"],
        ...
        await db.orders.update_one({"user_id": user_id, "id": assignment.get("order_id")}, {"$set": order_patch})

        await db[PAYMENT_EVENTS].insert_one({
            "id": str(uuid.uuid4()), "user_id": user_id, "assignment_id": assignment_id,
            "order_id": assignment.get("order_id"), "driver_id": assignment.get("driver_id"),
            "decision": payload.decision, "note": normalize_text(payload.note),
            "payment_method": review.get("payment_method"), "amount": review.get("amount"),
            "actor_id": normalize_text(actor.get("id")), "occurred_at": now,
        })
        return {
            "assignment_id": assignment_id,
            "decision": payload.decision,
            "payment_status": assignment_patch["payment_status"],
...
__all__ = ["ReviewPayload", "ensure_store_delivery_payment_review_indexes", "make_store_delivery_payment_review_router"]

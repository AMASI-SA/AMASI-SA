...
def make_order_review_mezan_image_unlink_router(
    ...
    async def unlink_and_delete_mezan_image(
        ...
        # Never clear another order silently.  The operator must open that
        # order and choose a replacement image there first.
        ...
        # A concurrent reference created after the preflight still blocks the
        # physical deletion.  The image remains available but is already safe
        # to retry after resolving that reference.

...
URL-based additions through Salla's Product Images endpoints. Every publish
...
def normalize_media_rows(rows: Any) -> list[dict[str, Any]]:
    ...
    for index, raw in enumerate(rows):
        ...
        result.append({
            ...
            "is_main": is_main,
            "sort": index + 1,
            "source": "salla" if image_id else "external_url",
        })
    if main_count != 1:
...
async def _salla_multipart(db: Any, user_id: str, method: str, path: str, fields: dict[str, Any] | None = None) -> dict[str, Any]:
    token = await ensure_fresh_access_token(db, user_id, recover_needs_reauth=True)
    files = {key: (None, str(value)) for key, value in (fields or {}).items() if value is not None}
    async with httpx.AsyncClient(timeout=httpx.Timeout(45.0)) as client:
        response = await client.request(method, f"{SALLA_API_BASE}{path}", headers={"Authorization": f"Bearer {token}"}, files=files or None)
...
def make_product_media_draft_router(db: Any, current_user: Callable) -> APIRouter:
    ...
    async def media_control(product_id: str, user: dict = Depends(current_user)) -> dict[str, Any]:
        ...
        draft = await db[MEDIA_DRAFTS].find_one({
            "user_id": str(user["id"]), "salla_product_id": str(product["salla_product_id"]), "status": {"$in": ["draft", "approved"]},
    ...
    async def save_media_draft(product_id: str, payload: dict = Body(...), user: dict = Depends(current_user)) -> dict[str, Any]:
        ...
        if previous:
            await db[MEDIA_DRAFTS].update_many({"user_id": str(user["id"]), "salla_product_id": str(product["salla_product_id"]), "status": {"$in": ["draft", "approved"]}}, {"$set": {"status": "superseded", "superseded_at": now}})
        draft = {
            "id": uuid.uuid4().hex,
            "user_id": str(user["id"]),
        ...
        await _audit(db, user, "product_media_draft_saved", str(product["salla_product_id"]), current, draft)
    ...
    async def approve_media_draft(product_id: str, draft_id: str, user: dict = Depends(current_user)) -> dict[str, Any]:
        ...
        product = await _product(db, str(user["id"]), product_id)
        draft = await db[MEDIA_DRAFTS].find_one({"id": draft_id, "user_id": str(user["id"]), "salla_product_id": str(product["salla_product_id"]), "status": "draft"}, {"_id": 0})
        if not draft:
            raise HTTPException(status_code=404, detail={"code": "media_draft_not_found_or_not_draft"})
        now = _now()
        await db[MEDIA_DRAFTS].update_one({"id": draft_id}, {"$set": {"status": "approved", "approved_by": str(user["id"]), "approved_by_name": user.get("name"), "approved_at": now, "updated_at": now}})
        draft.update({"status": "approved", "approved_by": str(user["id"]), "approved_by_name": user.get("name"), "approved_at": now, "updated_at": now})
        await _audit(db, user, "product_media_draft_approved", str(product["salla_product_id"]), None, draft)
        return {"ok": True, "draft": draft}
    ...
    async def publish_media_draft(product_id: str, draft_id: str, user: dict = Depends(current_user)) -> dict[str, Any]:
        ...
        product = await _product(db, str(user["id"]), product_id)
        draft = await db[MEDIA_DRAFTS].find_one({"id": draft_id, "user_id": str(user["id"]), "salla_product_id": str(product["salla_product_id"]), "status": "approved"}, {"_id": 0})
        if not draft:
        ...
        try:
            for removed in diff["removed"]:
                await _salla_multipart(db, str(user["id"]), "DELETE", f"/products/images/{removed['id']}")
            for row in after:
                fields = {"default": 1 if row.get("is_main") else 0, "sort": row.get("sort"), "alt": row.get("alt") or ""}
                if row.get("id"):
                    await _salla_multipart(db, str(user["id"]), "POST", f"/products/images/{row['id']}", fields)
                else:
                    await _salla_multipart(db, str(user["id"]), "POST", f"/products/{product['salla_product_id']}/images", {"photo": row["url"], **fields})
        except SallaError as exc:
            await db[MEDIA_DRAFTS].update_one({"id": draft_id}, {"$set": {"last_error": str(exc), "last_error_at": _now()}})
            await _audit(db, user, "product_media_publish_failed", str(product["salla_product_id"]), before, after, status="failed")
            raise HTTPException(status_code=exc.status_code or 502, detail={"code": "salla_media_publish_failed", "message": str(exc)}) from exc
        now = _now()
        version = {"id": uuid.uuid4().hex, "user_id": str(user["id"]), "salla_product_id": str(product["salla_product_id"]), "before_images": before, "after_images": after, "draft_id": draft_id, "published_by": str(user["id"]), "published_by_name": user.get("name"), "published_at": now}
        await db[MEDIA_VERSIONS].insert_one(dict(version))
        await db[MEDIA_DRAFTS].update_one({"id": draft_id}, {"$set": {"status": "published", "published_at": now, "published_by": str(user["id"]), "updated_at": now}})
        await db[PRODUCTS].update_one({"user_id": str(user["id"]), "salla_product_id": str(product["salla_product_id"])}, {"$set": {"images": after, "main_image": next((row["url"] for row in after if row.get("is_main")), after[0]["url"]), "details_synced_at": now, "updated_at": now}})
        await _audit(db, user, "product_media_published", str(product["salla_product_id"]), before, after)
        return {"ok": True, "version": version, "published_images": after}

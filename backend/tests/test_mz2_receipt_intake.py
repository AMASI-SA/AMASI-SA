...
class IntakeTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_both_input_orders_and_no_financial_write(self):
        ...
        linked = await self.link(receipt)
        self.assertEqual(linked.status_code, 200)
        self.assertNotIn('bank_receipt_required', [r['code'] for r in linked.json()['review_reasons']])
        saved = await self.db.accounting_settlements_v2.find_one({'id': 'draft'})
        self.assertNotIn('bank_receipt_required', [r['code'] for r in saved['review_reasons']])

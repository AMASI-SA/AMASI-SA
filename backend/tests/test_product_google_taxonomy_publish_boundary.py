...
TARGET = "Apparel & Accessories > Jewelry > Necklaces"
...
def test_taxonomy_publish_reads_before_and_after_then_records_synced(monkeypatch):
    calls = []
    reads = iter([
        {"data": {"google_taxonomy": None}},
        {"data": {"google_taxonomy": TARGET}},
    ])
    ...
    assert update["salla_google_taxonomy"] == TARGET


def test_taxonomy_publish_stays_failed_when_salla_readback_does_not_match(monkeypatch):
    calls = []

...
def test_tabby_product_column_payout_fee_is_separate_from_order_entries():
    ...
    assert all(entry["order_number"].isdigit() for entry in result["entries"] if entry["event_type"] != "settlement_fee")

...
never choose a bank per order. They upload the receipt, enter the received
amount/date/reference, then explicitly approve arrival.
...
async def save_receipt_draft(
    ...
    *,
    ...
    day = _source_day(transfer_date)
    ref = _clean(bank_reference)
    if len(ref) > 200:
        raise BankTransferError("bank_reference_too_long")
    ...
    draft = {
        ...
        "amount_matches": received == expected,
        "transfer_date": day,
        "bank_reference": ref or None,
        "bank_reference_key": ref.upper() if ref else None,
        "receipt_file_id": file_id,
        "receipt_file_sha256": file_sha256,
        "receipt_filename": filename,
        "status": "pending_approval" if received == expected else "needs_review",
        "review_reasons": [] if received == expected else ["received_amount_mismatch"],
...
async def approve_receipt(
    ...
    *,
    ...
    async def commit(scoped):
        ...
        if review.get("status") != "pending_approval" or not review.get("amount_matches"):
        ...
        received = Decimal(review["received_amount"])
        if received != _expected_amount(evidence):
            raise BankTransferError("received_amount_mismatch")
        received_at = _day_instant(review["transfer_date"])
        ...
        await _receipt_duplicate_guard(
            ...
            bank_reference=review.get("bank_reference") or "",
        )
        ...
        if prior and prior.get("status") == "posted":
            ...
            facts = {
                ...
                "order_number": evidence["order_number"],
                "bank_account_id": review["bank_account_id"],
                "amount": review["received_amount"],
                "transfer_date": review["transfer_date"],
        ...
        if (
            ...
            sale = await _post_sale_from_advance(
                ...
                actor=actor,
                evidence=evidence,
                review={**review, "advance_id": advance_id},
            )

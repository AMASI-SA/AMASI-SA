"""Owner-only native Meta Business OAuth routes for Apps & Integrations V2."""
...
from .meta_discovery import discover_meta_assets
from .meta_oauth_security import (
    ...
    _consume_state,
    _exchange_code,
    _exchange_long_lived_token,
    _iso,
    _redirect_uri,
    _state_secret,
    debug_meta_token,
    ensure_meta_connection_indexes,
    meta_oauth_configured,
    start_meta_connection,
)
from .meta_projection import persist_meta_projection

META_BINDING_COOKIE = "mezan_meta_oauth_binding"
META_BINDING_MAX_AGE = 15 * 60
META_CALLBACK_PATH = "/api/integrations-v2/meta/callback"


def _browser_binding(state_token: str) -> str:
    return hmac.new(
        _state_secret().encode("utf-8"),
        f"meta-browser:{state_token}".encode("utf-8"),
        hashlib.sha256,
    ).hexdigest()
...
async def handle_meta_callback(
    ...
    *,
    ...
    try:
        ...
        short_payload = await _exchange_code(code)
        long_payload = await _exchange_long_lived_token(
            str(short_payload["access_token"])
        )
        access_token = str(long_payload["access_token"])
        debug_data = await debug_meta_token(access_token)
        ...
        try:
            discovery = await discover_meta_assets(access_token)
        except Exception as exc:  # noqa: BLE001
            discovery_error = str(exc)
        await persist_meta_projection(
            db,
            user_id=str(state_payload["user_id"]),
            token_payload=long_payload,
        ...
        return RedirectResponse(
            _callback_redirect(outcome="error", code="meta_oauth_failed"),
            status_code=302,
        )
    return RedirectResponse(_callback_redirect(outcome="connected"), status_code=302)


async def test_meta_connection(db: Any, user_id: str) -> dict[str, Any]:
    now_iso = _iso()
    ...
    message = (
        "Meta Business OAuth connection and ad accounts are present."
        if connected and account_count
        else "Meta OAuth is connected, but no ad account was discovered."
        if connected
        else "No verified native Meta Business OAuth connection exists."
    )
    await db.mezan_integration_sync_runs_v2.insert_one(
        {
            ...
            "summary": {
                ...
                "asset_counts": (integration or {}).get("asset_counts") or {},
...
def install_meta_connection_actions() -> None:
    from . import service as service_module

    original_actions = service_module._actions
    if getattr(original_actions, "_mezan_meta_connection_actions", False):
        return
    ...
    def wrapped_actions(definition: Any, snapshot: dict) -> dict:
        ...
        connect_action = {
            ...
            "reason": (
                None
                if configured
                else "يجب ضبط إعدادات Meta Business OAuth في Backend أولًا."
            ),
            "href": None,
        }
        ...
        actions["settings"] = {
            "enabled": False,
            "reason": "إعدادات تطبيق Meta محفوظة كأسرار منصة ولا تُدخل لكل متجر.",
            "href": None,
        }
        actions["test_connection"] = {
            ...
            "reason": (
                ...
                else "اربط Meta Business أولًا قبل تشغيل الفحص المحلي."
            ),
            "href": None,
        }
        actions["sync_data"] = {
            "enabled": False,
            "reason": "مزامنة تقارير Meta الأصلية قيد النقل إلى بيانات ميزان 2؛ لن نستخدم موصل القديم.",
            "href": None,
        }
        return actions

    wrapped_actions._mezan_meta_connection_actions = True  # type: ignore[attr-defined]
    service_module._actions = wrapped_actions


def install_meta_stale_error_filter() -> None:
    from . import service as service_module
    ...
    if getattr(original, "_mezan_meta_stale_error_filter", False):
        return
    ...
    async def wrapped_v2_snapshot(self: Any, user_id: str, definition: Any):
        ...
        if (
            snapshot.get("provider") == META_PROVIDER_ID
            and snapshot.get("has_data")
            and isinstance(latest_error, dict)
            and str(latest_error.get("code") or "").startswith("meta_discovery_")
        ):
    ...
    wrapped_v2_snapshot._mezan_meta_stale_error_filter = True  # type: ignore[attr-defined]
    service_class._v2_snapshot = wrapped_v2_snapshot


def attach_meta_connection_routes(
    router: APIRouter,
    db: Any,
    current_user: Callable,
    require_owner: Callable[[Any], dict],
) -> None:
    install_meta_connection_actions()
    install_meta_stale_error_filter()

    @router.post("/meta/connect/start")
    async def meta_connect_start(
        response: Response,
        ...
        if not os.environ.get("FRONTEND_URL", "").strip():
            raise HTTPException(
                status_code=503,
                detail={
                    "code": "meta_oauth_frontend_url_missing",
                    "message": "يجب ضبط FRONTEND_URL في بيئة Backend أولًا.",
                    "missing": ["FRONTEND_URL"],
                },
            )
        result = await start_meta_connection(db, str(owner["id"]))
        state_token = _state_from_authorization_url(result["authorization_url"])
        if not state_token:
            raise HTTPException(
                status_code=500,
                detail={
                    "code": "meta_oauth_state_generation_failed",
                    "message": "تعذر إنشاء جلسة ربط Meta الآمنة.",
                },
            )
        response.set_cookie(
            META_BINDING_COOKIE,
            _browser_binding(state_token),
            max_age=META_BINDING_MAX_AGE,
            secure=_secure_cookie(),
            httponly=True,
            samesite="lax",
            path=META_CALLBACK_PATH,
    ...
    async def meta_callback(request: Request) -> RedirectResponse:
        response = await handle_meta_callback(
            ...
            browser_binding=request.cookies.get(META_BINDING_COOKIE),
        )
        _clear_binding_cookie(response)
        return response

    @router.post(
        f"/{META_PROVIDER_ID}/test-connection",
        name="test_meta_ads_connection",
    )
    async def meta_local_test(user: dict = Depends(current_user)) -> dict:
        owner = require_owner(user)
        return await test_meta_connection(db, str(owner["id"]))

...
account projections.  It never reads access tokens and performs no provider,
campaign, accounting, or Qoyod writes.  A successful native reporting sync
recreates the missing provider-level projection through the existing reporting
service.
"""
...
async def augment_auto_sync_targets(
    ...
    """Union normal scheduler targets with recoverable Meta account targets."""
    targets = {
...
def install_ads_auto_sync_target_recovery() -> None:
    ...
    if getattr(original, "_mezan_meta_target_recovery_v1", False):
        return

    async def wrapped_targets(db: Any) -> list[tuple[str, str]]:
        return await augment_auto_sync_targets(db, await original(db))

    wrapped_targets._mezan_meta_target_recovery_v1 = True  # type: ignore[attr-defined]

...
The automatic worker already calls Salla ``resync_single_order`` before every
send.  The resync writes the current payment facts to ``unified_orders`` under
the Orders owner, while ``manual_send_one`` historically loaded the newest
``integration_inbox`` row under the legacy ``main`` tenant.  For orders that
started as pending-payment, that legacy row could still contain the creation
snapshot and make the sender treat a paid Tamara/Tabby order as unpaid.

This module closes that split-brain window.  Immediately before the Plan-B
sender executes, it copies the authoritative payment/status facts produced by
the Salla resync into the exact legacy inbox row that ``manual_send_one`` will
read.  Missing or still-pending facts fail closed; no Qoyod write is attempted.
"""
...
async def sync_authoritative_payment_to_inbox(
    ...
    *,
    ...
    """Copy the fresh Salla-derived payment state into the sender's inbox row.

    ``resync_single_order`` must run before this function.  The unified order is
    then the authoritative source because its merge policy marks current payment
    fields as latest-wins.  We update only payment/status fields on the legacy
    inbox row, preserving its canonical items, totals, idempotency markers, and
    audit history.
    """
    ...
    # A completed order that still resolves to a pending placeholder is unsafe.
    # Do not fall back to the creation snapshot and do not guess a payment method.
    ...
    for field in (*_PAYMENT_FIELDS, *_STATUS_FIELDS):
        ...
        # Zero/False are valid payment facts; only skip genuinely absent values.
...
def install_auto_send_payment_freshness_patch() -> None:
    """Patch only the Plan-B automatic sender; manual UI behavior is unchanged."""

"""Closed-cohort recovery state machine; NOT connected to the live worker.

The production adapter/activation UI must be reviewed separately. Importing
...
async def recover_one(scope: Scope, reference: str, ports: Ports) -> Outcome:
    ...
    try:
        ...
        reason = str(exc) if isinstance(exc, EvidenceError) else "outcome_unknown"
...
async def audit_one(scope: Scope, reference: str, ports: Ports) -> Outcome:
    ...
    try:
        ...
        result = Outcome(reference, "review",
                         str(exc) if isinstance(exc, EvidenceError) else "outcome_unknown")
    await ports.finish(result)
    return result

...
def _require_customer_service(user: Any) -> dict[str, Any]:
    ...
    allowed = (
        ...
        or permission in extra
    ) and permission not in denied
    if not allowed:
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail={"code": "delivery_instruction_permission_required"},
        )
    return user
...
def _validate_schedule(payload: Any) -> None:
    ...
    if instruction_type == "scheduled" and not payload.delivery_date:
...
def make_store_delivery_customer_instruction_router(db: Any, current_user: Callable[..., Any]) -> APIRouter:
    ...
    async def create_instruction(payload: DeliveryInstructionCreate, user: dict = Depends(current_user)) -> dict[str, Any]:
        ...
        await ensure_store_delivery_instruction_indexes(db)

        assignment = await db.store_delivery_assignments.find_one(
            {"user_id": user_id, "order_id": normalize_text(payload.order_id), "status": {"$ne": "cancelled"}},
        ...
        await _event(
            ...
            payload={"order_id": doc["order_id"], "driver_id": doc["driver_id"]},
        )
        doc.pop("_id", None)
    ...
    async def update_instruction(instruction_id: str, payload: DeliveryInstructionUpdate, user: dict = Depends(current_user)) -> dict[str, Any]:
        actor = _require_customer_service(user)
        user_id = _merchant_user_id(actor)
        current = await db[STORE_DELIVERY_INSTRUCTIONS].find_one(
            {"user_id": user_id, "id": instruction_id, "status": "active"}, {"_id": 0}
        )
        if not current:
        ...
        merged = DeliveryInstructionCreate(
            ...
            instruction_type=payload.instruction_type or current["instruction_type"],
            priority=payload.priority or current["priority"],
            note=current.get("note", "") if payload.note is None else payload.note,
        ...
        now = _now()
        patch = {
            "instruction_type": merged.instruction_type,
            "priority": merged.priority,
            "note": normalize_text(merged.note),
            ...
            "updated_at": now,
            "updated_by": normalize_text(actor.get("id")),
        }
        result = await db[STORE_DELIVERY_INSTRUCTIONS].find_one_and_update(
            {"user_id": user_id, "id": instruction_id, "version": payload.expected_version},
            {"$set": patch},
        ...
        await _event(
            ...
            user_id=user_id,
            instruction_id=instruction_id,
            event_type="delivery_instruction_updated",
            actor_id=normalize_text(actor.get("id")),
            payload={"previous_version": payload.expected_version},
        )
        return result
    ...
    async def order_instructions(order_id: str, user: dict = Depends(current_user)) -> dict[str, Any]:
        ...
        items = await db[STORE_DELIVERY_INSTRUCTIONS].find(
            {"user_id": user_id, "order_id": normalize_text(order_id)},
            {"_id": 0, "user_id": 0},
    ...
    async def my_instructions(
        ...
        owner_id = normalize_text(actor.get("created_by"))
        driver = await db.store_drivers.find_one(
            {"user_id": owner_id, "account_user_id": normalize_text(actor.get("id")), "status": "active"},
            {"_id": 0, "id": 1},
        )
        if not driver:
        ...
        items = await db[STORE_DELIVERY_INSTRUCTIONS].find(query, {"_id": 0, "user_id": 0}).sort(
            [("delivery_date", 1), ("delivery_time", 1), ("created_at", 1)]
        ).to_list(length=500)
        return {"items": items, "total": len(items)}
    ...
    async def acknowledge_instruction(instruction_id: str, user: dict = Depends(current_user)) -> dict[str, Any]:
        ...
        await _event(
            ...
            user_id=owner_id,
            instruction_id=instruction_id,
            event_type="delivery_instruction_acknowledged",
            actor_id=normalize_text(actor.get("id")),
...
__all__ = [
    "DeliveryInstructionCreate",
    "DeliveryInstructionUpdate",
    "ensure_store_delivery_instruction_indexes",
    "make_store_delivery_customer_instruction_router",
]

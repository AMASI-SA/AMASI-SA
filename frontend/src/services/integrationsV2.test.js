...
test("filters and capability summaries support the control-centre tabs", () => {
    ...
    expect(filterIntegrationProviders(overview.providers, { status: "api_connection" })
        .map((row) => row.provider)).toEqual(["salla", "meta_ads"]);
...
test("connection provenance produces exact independent counters", () => {
    ...
    expect(
        overview.summary.api_connections
        + overview.summary.legacy_integrations
        + overview.summary.data_feeds
        + overview.summary.disconnected
        + overview.summary.planned
        + overview.summary.unknown,
    ).toBe(overview.summary.total);
    expect(filterIntegrationProviders(overview.providers, { status: "data_feed" })
        .map((row) => row.provider)).toEqual(["tiktok_ads"]);
    expect(filterIntegrationProviders(overview.providers, { status: "disconnected" }))
        .toHaveLength(4);
});
...
test("unknown connection provenance has its own counter and filter", () => {
    const overview = normalizeIntegrationOverview({
        providers: [{
            ...
            connection_provenance: "unknown",
        }],
    });

    expect(overview.summary.unknown).toBe(1);
    expect(filterIntegrationProviders(overview.providers, { status: "unknown" })
        .map((row) => row.provider)).toEqual(["google_ads"]);
    expect(
        overview.summary.api_connections
        + overview.summary.legacy_integrations
        + overview.summary.data_feeds
...
test("unknown permissions are not counted missing and action links stay internal", () => {
    const overview = normalizeIntegrationOverview({
        providers: [{
            ...
            actions: {
                settings: { enabled: true, href: "/settings?tab=meta" },
                reconnect: { enabled: true, href: "https://evil.example/steal" },
                disconnect: { enabled: true, href: "javascript:alert(1)" },
            },
        }],
    });
    const meta = overview.providers.find((row) => row.provider === "meta_ads");

...
test("keeps the Riyadh rollover waiting state instead of manufacturing confirmed zeros", () => {
    const result = mergeDashboardWithPlatformSpend(
        {
            totals: {
                total_sales: 0,
        ...
        },
        {
            ...
            spend_quality: {
                ...
                amount_available: false,
                reason_codes: ["open_day_provider_payload_waiting"],
            },
        },
    );
    ...
    expect(result.ads_v2.breakdown.snapchat).toBeNull();
    expect(result.ads_v2.breakdown.meta).toBeNull();
    expect(result.ads_v2.breakdown.tiktok).toBeNull();
    expect(result.ads_v2.breakdown.google).toBeNull();
    expect(result.ads_v2.spend_quality.status).toBe("incomplete");
    expect(result.ads_v2.spend_quality.meta.data_state).toBe("waiting_incomplete");
});

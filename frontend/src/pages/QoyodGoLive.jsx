            ...
            {blocking > 0 && Array.isArray(extra.sample_blocking) &&
             extra.sample_blocking.length > 0 && (
              <details className="bg-rose-50 border border-rose-200 rounded p-2"
                ...
                <ul className="mt-2 space-y-2 text-[10px]" dir="ltr">
                  {extra.sample_blocking.map((s) => (
                    <li key={s.row_id} className="text-rose-900 bg-white/60 border border-rose-200 rounded p-1.5 font-mono">
                      <div>
                        <strong>#{s.order_number || "?"}</strong> · {s.last_failed_stage} · {s.error_code || "—"} · {s.trace_id?.slice(0, 8)}
                      </div>
                      <div className="text-[10px] mt-0.5 opacity-80">
                        attempts {s.requeue_attempts ?? 0}/{s.max_requeue_attempts ?? 2}
                        {" · "}
                        reason: <strong>{s.requeue_reason || "—"}</strong>
                        {s.matched_pattern && <> · pattern: {s.matched_pattern}</>}
                      </div>
                      {s.error_message && (
                        <div className="text-[10px] mt-0.5 opacity-90 break-all" dir="ltr">
                          {s.error_message}
                        </div>
                      )}
                    </li>

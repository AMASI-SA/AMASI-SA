        ...
        <div className="space-y-5" dir="rtl" data-testid="marketing-platform-workspace">
            <header className="overflow-hidden rounded-3xl border border-slate-800 bg-slate-950 text-white shadow-xl">
                ...
                <div className="border-t border-white/10 bg-slate-900/80 px-5 py-3 text-xs font-bold text-slate-300 sm:px-7">
                    حالة الربط: <span className="text-emerald-200">{statusLabel(connection.status)}</span>
            ...
            {activeTab === "campaigns" && (
                <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm" data-testid="marketing-campaigns-table">
                    <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-4 py-4">
                        <div>
                            <h2 className="font-black text-slate-900">الحملات</h2>
                            <p className="mt-1 text-xs font-semibold text-slate-500">{numeric(pagination.total)} حملة ضمن البحث والفترة.</p>
                        </div>
                        <button type="button" disabled className="rounded-xl border border-slate-200 bg-slate-100 px-4 py-2 text-sm font-black text-slate-400" title="يتطلب دورة الاقتراح والاعتماد والتنفيذ والتحقق">
                            إنشاء حملة بالذكاء الاصطناعي — قريبًا
                        </button>
                    </div>
                        ...
                        <table className="min-w-[1250px] w-full text-right text-sm">
                            <thead className="bg-slate-50 text-xs font-black text-slate-600">
                                <tr>
                                    ...
                                    <th className="px-4 py-3">الصرف</th>
                                    ...
                                    <th className="px-4 py-3">الميزانية اليومية</th>
                                </tr>
                            </thead>
                                ...
                                {(data?.campaigns || []).map((campaign) => (
                                    <tr key={`${campaign.account_id}-${campaign.campaign_id}`} className="hover:bg-slate-50/70">
                                        <td className="px-4 py-3">
                                            <div className="max-w-[260px] font-black text-slate-900">{campaign.campaign_name}</div>
                                            <div className="mt-1 font-mono text-[10px] text-slate-400">{campaign.campaign_id}</div>
                                        </td>
                                        ...
                                        <td className="px-4 py-3 font-bold text-slate-600">{campaign.account_name}</td>
                                        <td className="px-4 py-3 font-mono font-black">{money(campaign.spend_sar)}</td>
                                        <td className="px-4 py-3 font-mono font-black">{numeric(campaign.orders)}</td>
                                        <td className="px-4 py-3 font-mono font-black">{money(campaign.sales_sar)}</td>
                                        <td className="px-4 py-3 font-mono font-black">{ratio(campaign.roas, "×")}</td>
                                        <td className="px-4 py-3 font-mono font-black">{money(campaign.cpa_sar)}</td>
                                        <td className="px-4 py-3 font-mono">{numeric(campaign.impressions)}</td>
                                        <td className="px-4 py-3 font-mono">{numeric(campaign.swipes)}</td>
                                        <td className="px-4 py-3 font-mono">{ratio(campaign.ctr_pct, "%")}</td>
                                        <td className="px-4 py-3 font-mono">{campaign.budget?.daily_native === null ? "غير متاح" : `${numeric(campaign.budget.daily_native)} ${campaign.budget.currency || ""}`}</td>
                                    </tr>
                                ))}
                                {!(data?.campaigns || []).length && (
                                    <tr><td colSpan="12" className="px-4 py-12 text-center font-bold text-slate-500">لا توجد حملات موثقة ضمن الفترة أو البحث.</td></tr>
                                )}
                            </tbody>
                        </table>
                    </div>
                    {pagination.pages > 1 && (
                        <div className="flex items-center justify-center gap-3 border-t border-slate-100 p-4">
                            <button type="button" disabled={page <= 1} onClick={() => setPage((value) => Math.max(1, value - 1))} className="rounded-xl border px-4 py-2 font-black disabled:opacity-40">السابق</button>
                            <span className="font-mono text-sm font-black">{pagination.page} / {pagination.pages}</span>
                            <button type="button" disabled={page >= pagination.pages} onClick={() => setPage((value) => value + 1)} className="rounded-xl border px-4 py-2 font-black disabled:opacity-40">التالي</button>
                        </div>
                    )}
                </section>
            )}
                    ...
                    {(data?.accounts || []).map((account) => (
                        <article key={account.account_id} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                            <div className="flex items-start justify-between gap-3">
                                ...
                                <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-black text-emerald-700">{account.currency || "عملة غير معروفة"}</span>
                            </div>
                            <div className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-4">
                                <div className="rounded-xl bg-slate-50 p-3"><div className="text-[10px] font-black text-slate-500">الصرف</div><div className="mt-1 font-mono font-black">{money(account.spend_sar)}</div></div>
                                <div className="rounded-xl bg-slate-50 p-3"><div className="text-[10px] font-black text-slate-500">الطلبات</div><div className="mt-1 font-mono font-black">{numeric(account.orders)}</div></div>
                                <div className="rounded-xl bg-slate-50 p-3"><div className="text-[10px] font-black text-slate-500">المبيعات</div><div className="mt-1 font-mono font-black">{money(account.sales_sar)}</div></div>
                                <div className="rounded-xl bg-slate-50 p-3"><div className="text-[10px] font-black text-slate-500">ROAS</div><div className="mt-1 font-mono font-black">{ratio(account.roas, "×")}</div></div>
                            </div>
                        </article>
                    ))}
                    {!(data?.accounts || []).length && (
                        <div className="rounded-2xl border border-dashed border-slate-200 bg-white p-12 text-center font-bold text-slate-500 lg:col-span-2">لا توجد بيانات أداء موزعة على الحسابات.</div>
                    )}
                </section>
            )}
                ...
                <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_360px]">
                    <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
                        <div className="flex items-center gap-3">
                            <span className="rounded-2xl bg-violet-100 p-3 text-violet-700"><Robot size={26} weight="duotone" /></span>
                            <div>
                    ...
                    <aside className="rounded-2xl border border-slate-800 bg-slate-950 p-5 text-white shadow-xl">
                        <div className="flex items-center gap-2 text-emerald-200"><ShieldCheck size={22} weight="fill" /><span className="font-black">حوكمة التنفيذ</span></div>

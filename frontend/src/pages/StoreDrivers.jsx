...
import { Plus, MapPin, Phone, CurrencyCircleDollar, Truck, PencilSimple } from "@phosphor-icons/react";
import { toast } from "sonner";

import { createStoreDriver, listStoreDrivers, updateStoreDriver } from "../services/storeDelivery";
...
function DriverForm({ initial, onClose, onSaved }) {
  const [form, setForm] = useState(initial ? {
    ...
    delivery_fee: String(initial.delivery_fee ?? 20),
  ...
  async function submit(event) {
    ...
    if (!form.name.trim() || !form.phone.trim() || !form.city.trim()) {
      toast.error("الاسم والجوال والمدينة مطلوبة");
      return;
    }
    setBusy(true);
    try {
      ...
      if (initial) {
        ...
        await createStoreDriver(payload);
      }
...
export default function StoreDrivers() {
  ...
  async function load() {
    setLoading(true);
    try {
      const data = await listStoreDrivers();
      setItems(data.items || []);
    } catch {
      toast.error("تعذر تحميل موصلي المتجر");
    } finally {
      setLoading(false);
    }
  }
  ...
  return (
    <main className="p-4 sm:p-6" dir="rtl">
      <div className="mb-5 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div><h1 className="text-2xl font-black text-slate-950">موصلو المتجر</h1><p className="mt-1 text-sm font-bold text-slate-500">إدارة موصلي أماسي ونطاق المدينة وسعر التوصيلة الحالي.</p></div>
        ...
        <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
          {items.map((driver) => (
            <article key={driver.id} className="rounded-2xl border bg-white p-5 shadow-sm">
              <div className="flex items-start justify-between gap-3"><div><div className="flex items-center gap-2 text-lg font-black"><Truck size={22} weight="duotone" />{driver.name}</div><div className={`mt-2 inline-flex rounded-full px-3 py-1 text-[11px] font-black ${driver.status === "active" ? "bg-emerald-100 text-emerald-800" : "bg-slate-200 text-slate-700"}`}>{driver.status === "active" ? "نشط" : "موقوف"}</div></div><button onClick={() => { setEditing(driver); setFormOpen(true); }} className="rounded-xl border p-2"><PencilSimple size={18} /></button></div>

...
function Badge({ ok, children }) {
    return (
        <span className={`inline-flex items-center rounded-full px-2 py-1 text-xs font-bold border ${ok ? "bg-emerald-50 text-emerald-700 border-emerald-200" : "bg-red-50 text-red-700 border-red-200"}`}>
            {children}
        </span>
    );
}
...
export default function QoyodProducts() {
    ...
    const fillFromRow = (row) => {
        setForm({
            sku: row.sku || "",
            qoyod_product_id: String(row.qoyod_product_id || "").startsWith("DRY:") || String(row.qoyod_product_id || "").startsWith("PREVIEW:") ? "" : (row.qoyod_product_id || ""),
    ...
    const submitAdoption = async (e) => {
        ...
        try {
            ...
            setForm({ sku: "", qoyod_product_id: "", qoyod_product_name: "", note: "" });
            await load();
        } catch (e) {
    ...
    return (
        <div className="space-y-6" dir="rtl" data-testid="qoyod-products-page">
            <div className="flex items-start justify-between flex-wrap gap-4">
                <div>
                    ...
                    <p className="text-sm text-slate-600 mt-2 leading-7 max-w-3xl">
                        هذه الصفحة تدير ربط SKU بين سلة/ميزان ومنتجات قيود. الاعتماد هنا محلي داخل ميزان فقط ولا يرسل طلب إنشاء أو تعديل إلى قيود.
                    </p>
                </div>
                <button
                    type="button"
                    onClick={load}
                    disabled={loading}
                    className="rounded-lg bg-brand text-white px-4 py-2 text-sm font-bold disabled:opacity-50"
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <Card title="حالة اتصال قيود" value={health?.credentials_loaded ? "متصل" : "غير متصل"} tone={health?.credentials_loaded ? "green" : "red"} />
                <Card title="إعدادات المنتج" value={defaultsOk ? "مكتملة" : "ناقصة"} tone={defaultsOk ? "green" : "red"} hint="التصنيف، الضريبة، الوحدة، حساب المبيعات" />
                <Card title="Mappings تحتاج إصلاح" value={fmtInt(dryMappings.length)} tone={dryMappings.length ? "amber" : "green"} hint="DRY/PREVIEW أو dry_run_only" />
                <Card title="نوع المنتج الافتراضي" value={settings?.default_product_type || health?.default_product_type || "—"} tone="blue" />
            </div>

            <div className={`rounded-xl border p-4 ${defaultsOk ? "bg-emerald-50 border-emerald-200" : "bg-red-50 border-red-200"}`}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mt-4">
                    {defaults.map((row) => (
                        <div key={row.key} className="rounded-lg bg-white border p-3">
                            ...
                            <div className="text-xs text-slate-500 mt-1" dir="ltr">{row.apiKey}</div>
                            <div className="text-sm font-bold mt-2 break-words num">{valuePreview(row.value)}</div>
                        </div>
                    ))}
                </div>
            </div>

            <form onSubmit={submitAdoption} className="rounded-xl border bg-white p-5 space-y-4" data-testid="qoyod-product-adopt-form">
                <div>
                    <h2 className="text-xl font-extrabold">اعتماد ربط منتج موجود في قيود</h2>
                    <p className="text-sm text-slate-600 mt-1 leading-6">
                        استخدمها عندما يكون المنتج موجوداً في قيود فعلاً وتريد ربطه بـ SKU من سلة. لا تضع رقم DRY أو PREVIEW هنا؛ أدخل رقم المنتج الحقيقي من قيود.
                    </p>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                    <Field label="SKU" value={form.sku} onChange={(v) => setForm((f) => ({ ...f, sku: v }))} required testid="adopt-sku" />
                    <Field label="Qoyod product id" value={form.qoyod_product_id} onChange={(v) => setForm((f) => ({ ...f, qoyod_product_id: v }))} required dir="ltr" testid="adopt-qoyod-id" />
                    <Field label="اسم المنتج في قيود" value={form.qoyod_product_name} onChange={(v) => setForm((f) => ({ ...f, qoyod_product_name: v }))} testid="adopt-name" />
                    <Field label="ملاحظة" value={form.note} onChange={(v) => setForm((f) => ({ ...f, note: v }))} testid="adopt-note" />
                </div>
                <div className="flex items-center gap-3 flex-wrap">
                    ...
                    <span className="text-xs text-slate-500">عملية محلية داخل ميزان فقط، ولا تستدعي Qoyod API.</span>
                </div>
            </form>
            ...
            <div className="rounded-xl border bg-white overflow-hidden">
                <div className="p-5 border-b flex items-center justify-between gap-3 flex-wrap">
                    <div>
                        ...
                        <p className="text-sm text-slate-500 mt-1">تعرض الصفوف التي تحمل DRY/PREVIEW أو dry_run_only=true.</p>
                    </div>
                    <input
                        value={query}
                ...
                {loading ? (
                    ...
                    <div className="p-10 text-center">
                        ...
                        <div className="font-extrabold text-slate-800">لا توجد mappings جافة تحتاج إصلاح</div>
                        <p className="text-sm text-slate-500 mt-2">إذا ظهر طلب متوقف بسبب منتج، سيظهر هنا عند وجود DRY/PREVIEW mapping.</p>
                    </div>
                ) : (
                        ...
                        <table className="min-w-full text-sm">
                            ...
                            <tbody>
                                {filteredDryMappings.map((row) => (
                                    <tr key={`${row.sku}-${row.qoyod_product_id}`} className="border-t hover:bg-slate-50">
                                        <td className="p-3 font-extrabold num" dir="ltr">{row.sku || "—"}</td>
                                        <td className="p-3 num" dir="ltr">{row.qoyod_product_id || "—"}</td>
                                        <td className="p-3">{row.qoyod_product_name || "—"}</td>
                                        <td className="p-3 text-amber-700 font-bold">{row.reason || "—"}</td>
                                        <td className="p-3">{row.source || "—"}</td>
                                        <td className="p-3">
                                            <button type="button" onClick={() => fillFromRow(row)} className="rounded-lg bg-blue-50 border border-blue-200 text-blue-700 px-3 py-1.5 text-xs font-bold hover:bg-blue-100">

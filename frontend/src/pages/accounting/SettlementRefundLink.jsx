...
export default function SettlementRefundLink({ draft, entries, canEdit, onLinked }) {
    ...
    const refunds = entries.filter(row => row.event_type === "refund");
    ...
    async function link(row) {
        ...
        try {
            await api.put(`/financial-provider-apps/accounting-module/settlements/drafts/${encodeURIComponent(draft.id)}/refund-match`,
                { entry_id: row.id, refund_id: values[row.id] });
            await onLinked();
        } catch (e) {
    ...
    return <section aria-label="مطابقة الاستردادات" className="space-y-3 rounded border p-3">
        ...
        {refunds.map(row => <div key={row.id} className="rounded border p-2">
            ...
            {row.refund_link ? <p>مرتبط بالاسترداد {row.refund_link.refund_id} — القيد {row.refund_link.txn_group_id}</p>
                : <><p>تحتاج مراجعة: هوية الاسترداد الأصلية وربط قيده غير مثبتين.</p>
                    {canEdit && <><label>معرف استرداد المزود الأصلي<input aria-label={`معرف استرداد ${row.id}`} value={values[row.id] || ""}

...
function EntitlementApproval({ row, busy, run }) {
    const [at, setAt] = useState("");
    const [reason, setReason] = useState("");
    const [evidence, setEvidence] = useState("");
    return <div className="space-y-2">
        <p>اعتمد فقط استحقاقًا مؤكدًا لبيع سبق إثباته. ينشأ التزام للعميل وصافي مرتجع وضريبة بتاريخ الاستحقاق؛ لا يُثبت دفعًا.</p>
        <label>تاريخ ووقت الاستحقاق مع المنطقة الزمنية<input aria-label="وقت استحقاق مؤكد" value={at} onChange={e=>setAt(e.target.value)} placeholder="YYYY-MM-DDTHH:mm:ss+03:00" /></label>
        <label>مرجع دليل الاستحقاق المؤكد<input aria-label="دليل استحقاق مؤكد" value={evidence} onChange={e=>setEvidence(e.target.value)} /></label>
        <label>سبب اعتماد الاستحقاق<input aria-label="سبب اعتماد الاستحقاق" value={reason} onChange={e=>setReason(e.target.value)} /></label>
        <button disabled={busy || !at.trim() || !reason.trim() || !evidence.trim()} onClick={()=>run(()=>api.post(base+"/"+row.id+"/recognize",{
            amount:row.amount,recognized_at:at,reason,evidence_ref:evidence}))}>اعتماد الاستحقاق وإثبات الالتزام {row.amount}</button>
    </div>;
}
...
export default function AccountingCustomerRefunds({ accountingPermissions = [] }) {
    ...
    return <section className="space-y-3 rounded-xl border p-4" dir="rtl">
        ...
        <p>الويبهوك ينشئ مسودة للمراجعة. اعتماد استحقاق مؤكد يثبت المرتجع وضريبته الأصلية والتزام العميل. اعتماد الحركة اليومية يسدد هذا الالتزام فقط، دون تكرار المرتجع والضريبة. إلغاء طلب قبل إثبات البيع لا ينشئ مرتجع إيراد. التحويل البنكي لا يخفض ذمة مزود الدفع.</p>
        ...
        {can("create") && <div className="grid gap-3 sm:grid-cols-2">
            ...
            <label>تاريخ المسودة المقترح للمراجعة<input aria-label="تاريخ استرداد العميل" type="date" value={date} onChange={e => setDate(e.target.value)} /></label>
        ...
        {data.cases.map(row => <article key={row.id} className="rounded border p-3">
            ...
            {row.accounting_version === 2 && <p>تاريخ الاستحقاق المحاسبي: {row.recognized_at} — وقت تسجيل الاعتماد: {row.confirmed_at}</p>}
            {!row.recognized && row.state !== "conflict" && can("recognize") && <EntitlementApproval row={row} busy={busy} run={run} />}
        </article>)}

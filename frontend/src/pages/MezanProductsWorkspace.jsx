    ...
    return (
        <div className="fixed inset-0 z-[120] flex items-center justify-center bg-slate-950/45 p-4" dir="rtl">
            <div className="max-h-[85vh] w-full max-w-3xl overflow-hidden rounded-3xl bg-white shadow-2xl">
                <div className="flex items-center justify-between border-b px-6 py-4"><div><h3 className="text-lg font-black">توليد SKU تلقائي</h3><p className="text-xs text-slate-500">ناقص SKU: {preview.missing_total}</p></div><button onClick={onClose}>✕</button></div>
                <div className="max-h-[55vh] space-y-2 overflow-auto p-5">{(preview.items || []).map((row) => <div key={row.salla_product_id} className="flex items-center justify-between rounded-xl border p-3"><span className="font-bold">{row.name}</span><span className="num font-black text-violet-800">{row.proposed_sku}</span></div>)}</div>
                <div className="flex justify-end border-t p-4"><button disabled={busy || !(preview.items || []).length} onClick={onApply} className="rounded-xl bg-violet-700 px-5 py-3 font-black text-white disabled:opacity-50">{busy ? "جارٍ التحديث…" : "اعتماد"}</button></div>
            </div>
        </div>
    );
}
...
export default function MezanProductsWorkspace() {
    ...
    const loadSelected = useCallback(async () => {
        ...
        try {
            const [detailResult, costResult] = await Promise.all([
            ...
            toast.error(err?.response?.data?.detail?.message || "تعذر جلب تفاصيل المنتج");
    ...
    async function applySkuBatch() {
        ...
        try {
            ...
            toast.success(`تم تحديث ${result.succeeded || 0} منتج`);
            setSkuPreview(null);
            await load({ page: 1 });
        ...
        <div className="space-y-4" dir="rtl">
            ...
            <section className="rounded-3xl border bg-white p-4 shadow-sm">
                <div className="flex flex-col gap-3 xl:flex-row">
                    <form onSubmit={(event) => { event.preventDefault(); setAppliedQuery(query.trim()); load({ page: 1, search: query.trim() }); }} className="relative flex-1">
                    ...
                    <select value={sort} onChange={(event) => setSort(event.target.value)} className="rounded-xl border px-3"><option value="newest">المضافة حديثًا</option><option value="oldest">الأقدم</option><option value="name">الاسم</option><option value="price_high">السعر الأعلى</option><option value="price_low">السعر الأقل</option></select>
                    <button onClick={() => setMissingSku((value) => !value)} className="rounded-xl border px-4">بدون SKU</button>
                    <button onClick={async () => setSkuPreview(await previewMissingSkus({ limit: 50 }))} className="rounded-xl border border-violet-300 px-4 text-violet-800">توليد SKU</button>
                    <button onClick={syncNow} disabled={syncing} className="rounded-xl bg-violet-700 px-4 text-white">{syncing ? <SpinnerGap className="inline animate-spin" /> : <ArrowsClockwise className="inline" />} مزامنة سلة</button>
                </div>
            </section>
            ...
            <section className="grid min-h-[820px] gap-4 xl:grid-cols-[220px_420px_minmax(0,1fr)]">
                ...
                <div className="overflow-hidden rounded-3xl border bg-white">
                    <div className="flex justify-between border-b p-4"><div><h2 className="font-black">المنتجات</h2><p className="text-xs text-slate-500">{pagination.total} منتج</p></div><SortAscending /></div>
                    <div className="max-h-[760px] overflow-auto">
                        {loading ? <div className="flex h-72 items-center justify-center"><SpinnerGap className="animate-spin" /></div> : items.map((product) => {
                            ...
                            return <button key={id} onClick={() => setSelectedId(id)} className={`flex w-full items-center gap-3 border-b p-3 text-right ${selectedId === id ? "bg-emerald-50" : ""}`}><ProductThumb product={product} /><div className="min-w-0 flex-1"><h3 className="truncate font-black">{product.name}</h3><p className="text-xs text-slate-500">{product.sku || "بدون SKU"} · {money(product.sale_price ?? product.price)}</p><span className="text-xs">{statusLabel(product.status)}</span></div></button>;
                        })}
                    </div>
                    <div className="flex justify-between border-t p-3"><button disabled={pagination.page <= 1} onClick={() => load({ page: pagination.page - 1 })}>السابق</button><span>{pagination.page} / {pagination.total_pages}</span><button disabled={pagination.page >= pagination.total_pages} onClick={() => load({ page: pagination.page + 1 })}>التالي</button></div>
                </div>

                <main className="rounded-3xl border bg-white p-5">
                    {detailLoading ? <div className="flex h-full items-center justify-center"><SpinnerGap className="animate-spin" /></div> : !selected ? <div className="flex h-full flex-col items-center justify-center text-slate-400"><Cube size={54} /><p>اختر منتجًا</p></div> : <div className="space-y-5">
                        <div className="flex items-center gap-4 border-b pb-5"><ProductThumb product={selected} size="lg" /><div><h1 className="text-xl font-black">{selected.name}</h1><p className="text-xs text-slate-500">Salla #{selected.salla_product_id} · {selected.sku || "بدون SKU"}</p></div></div>

                        <ProductControlCenterPanel productId={selectedId} product={selected} onPublished={loadSelected} />

                        <section className="rounded-2xl border p-4">
                            <h2 className="mb-3 font-black"><ImageSquare className="inline" /> الصور الحالية ({media.length})</h2>
                            <div className="grid grid-cols-3 gap-3 sm:grid-cols-5">{media.map((row) => <img key={row.id || row.url} src={row.url} alt={row.alt || ""} className="aspect-square w-full rounded-xl border object-cover" />)}</div>
                            <p className="mt-3 text-xs text-slate-500">إدارة رفع الصور وترتيبها وALT ستبنى على نفس مسار المسودة والاعتماد، ولا تُنفّذ كتعديل غير مراقب.</p>
                        </section>

                        <section className="rounded-2xl border p-4"><h2 className="mb-3 font-black">معاينة وصف المنتج الحالي</h2><DescriptionPreview html={selected.description_html || selected.description} /></section>

                        <section className="rounded-2xl border border-emerald-200 p-4">
                            <div className="mb-4 flex justify-between"><div><h2 className="font-black">التكلفة الأساسية — Mezan</h2><p className="text-xs text-slate-500">مستقلة عن نشر بيانات المنتج إلى سلة، وتُحتسب دائمًا لكل قطعة.</p></div><button onClick={saveCosts} disabled={costSaving} className="rounded-xl bg-emerald-700 px-4 py-2 text-white"><FloppyDisk className="inline" /> حفظ</button></div>
                            <div className="grid gap-4 md:grid-cols-2"><label className="text-sm">تكلفة سلة<input value={selected.cost_price_from_salla ?? ""} readOnly className="mt-1 w-full rounded-xl border bg-slate-50 p-3" /></label><label className="text-sm">تكلفة المنتج في ميزان<input type="number" min="0" step="0.01" value={costs.base_cost} onChange={(event) => setCosts((current) => ({ ...current, base_cost: event.target.value }))} className="mt-1 w-full rounded-xl border border-emerald-300 p-3" /></label></div>
                        </section>

                        <ProductOptionCostEditor productId={selectedId} options={options} customFields={customFields} />

                        <section className="rounded-2xl border p-4">
                            <h2 className="mb-3 font-black">المتغيرات ({variants.length})</h2>
                            {!variants.length ? <p className="text-slate-400">لا توجد متغيرات مستقلة.</p> : <div className="overflow-auto"><table className="w-full min-w-[700px] text-right text-xs"><thead><tr><th className="p-3">المتغير</th><th>SKU</th><th>السعر</th><th>تكلفة ميزان</th></tr></thead><tbody>{variants.map((variant) => <tr key={variant.id} className="border-t"><td className="p-3 font-bold">{variant.display_name || variant.name || `متغير #${variant.id}`}</td><td>{variant.sku || "—"}</td><td>{money(variant.price)}</td><td><input type="number" min="0" step="0.01" value={costs.variant_costs?.[variant.id] ?? ""} onChange={(event) => setCosts((current) => ({ ...current, variant_costs: { ...current.variant_costs, [variant.id]: event.target.value } }))} className="w-28 rounded-lg border p-2" /></td></tr>)}</tbody></table></div>}
                        </section>
                    </div>}
                </main>
            </section>

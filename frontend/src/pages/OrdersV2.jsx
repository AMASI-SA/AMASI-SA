                            ...
                            <button type="button" onClick={() => navigate(`/orders-v2/${encodeURIComponent(id)}`)} className="flex min-w-0 flex-1 items-center gap-3 text-right">
                                ...
                                <div className="min-w-0 flex-1">
                                    <div className="flex items-center gap-2">
                                        <div className="truncate text-[15px] font-semibold">{order.customer?.name || "عميل بدون اسم"}</div>
                                        {order.is_new && <span className="shrink-0 rounded-full border border-rose-300 px-2 py-0.5 text-xs font-bold text-rose-600">جديد</span>}
                                        {order.is_gift && <span className="rounded-full bg-emerald-500 px-2 py-1 text-[10px] font-bold text-white"><Gift size={14} /> إهداء</span>}
                                    </div>
                                    <div className="mt-1 flex flex-wrap items-center gap-x-2 text-xs text-slate-400"><span>#{id}</span><span>•</span><span>{cityName(order)}</span><span>•</span><span className="inline-flex items-center gap-1"><span className={`h-2 w-2 shrink-0 rounded-full ${statusVisual(status).dot}`} />{status}</span><span>•</span><span>{Number(order.items?.length || 0)} قطعة</span><span>•</span><span>{order.payment?.method_native || order.payment?.method || "غير محدد"}</span></div>
                                </div>
                                <div className="flex shrink-0 items-center gap-3"><div className="text-left"><span className="font-semibold text-teal-800">{formatMoney(order.totals?.total)}</span><div className="mt-1 text-xs text-slate-400">{formatOrderDate(order.created_at)}</div></div><CaretLeft size={18} className="text-slate-300" /></div>
                            </button>
                        </div>;
                    })}</div>
                )}

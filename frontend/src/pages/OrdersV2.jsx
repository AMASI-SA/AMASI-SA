...
function statusPriority(status) {
    ...
    const index = STATUS_PRIORITY.findIndex((aliases) => aliases.some((alias) => value === normalizedStatus(alias)));
...
function SourceBadge({ order }) {
    ...
    return <span title={`مصدر الطلب: ${badge.label}`} aria-label={`مصدر الطلب: ${badge.label}`} className={`inline-flex h-6 min-w-6 items-center justify-center rounded-full border px-1.5 text-[11px] font-black shadow-sm ${badge.className}`}>{badge.mark}</span>;
}
...
function CustomerAvatar({ customer }) {
    ...
    return <div className="relative flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-full bg-slate-100 text-slate-600">{fallback ? <span className="text-2xl leading-none">{fallback}</span> : <User size={24} weight="fill" />}{avatarUrl && <img src={avatarUrl} alt="" className="absolute inset-0 h-full w-full object-cover" loading="lazy" referrerPolicy="no-referrer" onError={(event) => { event.currentTarget.style.display = "none"; }} />}</div>;
}

function SelectionBox({ checked, disabled = false, onChange, label }) {
    return <button type="button" disabled={disabled} aria-label={label} aria-pressed={checked} onClick={(event) => { event.stopPropagation(); onChange?.(); }} className={`flex h-9 w-9 shrink-0 items-center justify-center rounded-lg border-2 transition ${checked ? "border-teal-700 bg-teal-50 text-teal-700" : "border-teal-700 bg-white text-transparent"} ${disabled ? "cursor-not-allowed opacity-30" : "hover:bg-teal-50"}`}><Check size={20} weight="bold" /></button>;
}
...
function CountCard({ label, count, active, onClick, isAll = false }) {
    ...
    return <button type="button" onClick={onClick} className={`min-w-[170px] rounded-2xl border bg-white px-4 py-4 text-right transition hover:-translate-y-0.5 hover:shadow-sm ${active ? visual.active : "border-slate-200"}`}><div className="flex items-start justify-between gap-3"><div className="text-xs font-bold text-slate-500">{label}</div><span className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-xl ${visual.iconBox}`}><Icon size={18} weight="duotone" /></span></div><div className="num mt-2 text-2xl font-extrabold text-slate-950">{Number(count || 0).toLocaleString("en-US")}</div></button>;
}
...
export default function OrdersV2() {
    ...
    return <div className="space-y-5" dir="rtl" data-testid="orders-v2-page">
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm">
            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between"><div className="flex items-center gap-2"><div className="rounded-xl bg-violet-100 p-2 text-violet-700"><Package size={24} weight="fill" /></div><div><h1 className="text-2xl font-extrabold text-slate-950">الطلبات</h1><p className="mt-1 text-sm text-slate-500">مركز الطلبات الموحد من Order Engine</p></div></div><div className="rounded-xl border border-violet-200 bg-violet-50 px-4 py-3"><div className="text-xs font-bold text-violet-700">الطلبات المعروضة</div><div className="num mt-1 text-2xl font-extrabold text-violet-950">{orders.length.toLocaleString("en-US")}</div></div></div>
        ...
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            ...
            {initialLoading ? <div className="flex min-h-72 items-center justify-center"><SpinnerGap size={32} className="animate-spin text-violet-600" /></div> : orders.length === 0 ? <div className="flex min-h-72 items-center justify-center text-slate-500">لا توجد طلبات مطابقة</div> : <div className="divide-y divide-slate-100">{orders.map((order) => {
                ...
                return <div key={id} className={`flex items-center gap-3 px-4 py-4 sm:px-5 sm:py-5 ${checked ? "bg-teal-50/60" : "hover:bg-slate-50"}`}><SelectionBox checked={checked} onChange={() => toggleOrder(id)} label={`تحديد الطلب ${id}`} /><button type="button" onClick={() => navigate(`/orders-v2/${encodeURIComponent(id)}`)} className="flex min-w-0 flex-1 items-center gap-3 text-right"><CustomerAvatar customer={order.customer} /><div className="min-w-0 flex-1"><div className="flex items-center gap-2"><div className="truncate text-[15px] font-semibold">{order.customer?.name || "عميل بدون اسم"}</div>{order.is_new && <span className="shrink-0 rounded-full border border-rose-300 px-2 py-0.5 text-xs font-bold text-rose-600">جديد</span>}{order.is_gift && <span className="rounded-full bg-emerald-500 px-2 py-1 text-[10px] font-bold text-white"><Gift size={14} /> إهداء</span>}</div><div className="mt-1 flex flex-wrap items-center gap-x-2 text-xs text-slate-400"><span>#{id}</span><span>•</span><span>{cityName(order)}</span><span>•</span><span className="inline-flex items-center gap-1"><span className={`h-2 w-2 shrink-0 rounded-full ${statusVisual(status).dot}`} />{status}</span><span>•</span><span>{Number(order.items?.length || 0)} قطعة</span><span>•</span><span>{order.payment?.method_native || order.payment?.method || "غير محدد"}</span></div></div><div className="flex shrink-0 items-center gap-3"><div className="text-left"><div className="flex items-center justify-end gap-2"><SourceBadge order={order} /><span className="font-semibold text-teal-800">{formatMoney(order.totals?.total)}</span></div><div className="mt-1 text-xs text-slate-400">{formatOrderDate(order.created_at, nowMs)}</div></div><CaretLeft size={18} className="text-slate-300" /></div></button></div>;
            })}</div>}

...
export default function ProductMediaDraftEditor({ productId, images = [], onPublished }) {
    ...
    async function uploadFile(file) {
        ...
        try {
            ...
            setDraftImages((rows) => normalizeMain([...rows, { ...result.image, is_main: rows.length === 0 }]));
    ...
    if (!editing) {
        return <section className="rounded-2xl border p-3 sm:p-4" data-testid="product-media-summary"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-black"><ImageSquare className="ml-1 inline" /> الصور الحالية ({current.length})</h2><p className="mt-1 text-xs text-slate-500">التعديل يمر بمسودة واعتماد قبل إرسال أي شيء إلى سلة.</p></div><button type="button" onClick={() => setEditing(true)} disabled={!permissions.edit} className="rounded-xl bg-violet-700 px-4 py-2 font-black text-white disabled:opacity-40">إدارة الصور</button></div><div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5">{current.map((row) => <div key={row.id || row.url} className="relative"><img src={row.url} alt={row.alt || ""} className="aspect-square w-full rounded-xl border object-cover" />{row.is_main && <span className="absolute right-2 top-2 rounded-full bg-emerald-600 px-2 py-1 text-[10px] font-black text-white">الرئيسية</span>}</div>)}</div>{draft && <div className="mt-3 rounded-xl border border-amber-200 bg-amber-50 p-3 text-xs font-bold text-amber-900">توجد مسودة صور بحالة: {draft.status}</div>}</section>;
    }

    return <section className="rounded-2xl border border-violet-200 bg-violet-50/30 p-3 sm:p-4" data-testid="product-media-editor">

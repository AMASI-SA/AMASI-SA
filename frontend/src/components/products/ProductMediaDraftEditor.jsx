...
import {
    ...
    PaperPlaneTilt, Plus, SpinnerGap, Trash, WarningCircle,
...
export default function ProductMediaDraftEditor({ productId, images = [], onPublished }) {
    ...
    function addUrl() {
        ...
        setDraftImages([...draftImages, { id: null, url, alt: "", is_main: draftImages.length === 0, sort: draftImages.length + 1 }]);
        setNewUrl("");
    }
    ...
    async function approve() {
        ...
        try {
            ...
            toast.error(error?.response?.data?.detail?.code || "تعذر اعتماد مسودة الصور");
        } finally {
            setBusy(false);
        }
    }
    ...
    async function publish() {
        ...
        try {
            ...
            toast.error(error?.response?.data?.detail?.message || "تعذر نشر الصور إلى سلة");
        } finally {
            setBusy(false);
        }
    }
        ...
        return (
            <section className="rounded-2xl border p-3 sm:p-4" data-testid="product-media-summary">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                    <div><h2 className="font-black"><ImageSquare className="ml-1 inline" /> الصور الحالية ({current.length})</h2><p className="mt-1 text-xs text-slate-500">التعديل يمر بمسودة واعتماد قبل إرسال أي شيء إلى سلة.</p></div>
                    <button type="button" onClick={() => setEditing(true)} disabled={!permissions.edit} className="rounded-xl bg-violet-700 px-4 py-2 font-black text-white disabled:opacity-40">إدارة الصور</button>
                </div>
                <div className="mt-3 grid grid-cols-2 gap-2 sm:grid-cols-3 md:grid-cols-4 xl:grid-cols-5">{current.map((row) => <div key={row.id || row.url} className="relative"><img src={row.url} alt={row.alt || ""} className="aspect-square w-full rounded-xl border object-cover" />{row.is_main && <span className="absolute right-2 top-2 rounded-full bg-emerald-600 px-2 py-1 text-[10px] font-black text-white">الرئيسية</span>}</div>)}</div>
    ...
    return (
        <section className="rounded-2xl border border-violet-200 bg-violet-50/30 p-3 sm:p-4" data-testid="product-media-editor">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                <div><h2 className="font-black"><ImageSquare className="ml-1 inline" /> إدارة صور المنتج</h2><p className="text-xs text-slate-500">المعروض الآن مسودة داخل ميزان حتى الاعتماد والنشر.</p></div>
                <button type="button" onClick={() => { setEditing(false); load(); }} className="rounded-xl border bg-white px-4 py-2 font-bold">إغلاق التحرير</button>
            </div>

            <div className="mt-4 flex flex-col gap-2 sm:flex-row"><input value={newUrl} onChange={(event) => setNewUrl(event.target.value)} placeholder="ألصق رابط صورة جديدة…" className="min-w-0 flex-1 rounded-xl border bg-white p-3" dir="ltr" /><button type="button" onClick={addUrl} className="rounded-xl border border-violet-300 bg-white px-4 py-3 font-black text-violet-800"><Plus className="inline" /> إضافة من رابط</button></div>

            <div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-3">
                {draftImages.map((row, index) => (
                    <article key={`${row.id || row.url}-${index}`} className="overflow-hidden rounded-2xl border bg-white">
                        <div className="relative"><img src={row.url} alt={row.alt || ""} className="aspect-square w-full object-cover" />{row.is_main && <span className="absolute right-2 top-2 rounded-full bg-emerald-600 px-2 py-1 text-[10px] font-black text-white">الرئيسية</span>}</div>
                        <div className="space-y-3 p-3">
                            <label className="block text-xs font-bold text-slate-600">ALT<input value={row.alt || ""} onChange={(event) => setDraftImages(draftImages.map((item, rowIndex) => rowIndex === index ? { ...item, alt: event.target.value } : item))} className="mt-1 w-full rounded-xl border p-2 text-sm" /></label>
                            <label className="flex items-center gap-2 text-xs font-bold"><input type="radio" name={`main-${productId}`} checked={row.is_main} onChange={() => setMain(index)} /> جعلها الصورة الرئيسية</label>
                            <div className="grid grid-cols-3 gap-2"><button type="button" onClick={() => move(index, -1)} disabled={index === 0} className="rounded-lg border p-2 disabled:opacity-30"><ArrowUp className="mx-auto" /></button><button type="button" onClick={() => move(index, 1)} disabled={index === draftImages.length - 1} className="rounded-lg border p-2 disabled:opacity-30"><ArrowDown className="mx-auto" /></button><button type="button" onClick={() => remove(index)} className="rounded-lg border border-rose-200 p-2 text-rose-600"><Trash className="mx-auto" /></button></div>
                        </div>
                    </article>
                ))}
            </div>
            ...
            <div className="mt-4 flex flex-wrap justify-end gap-2">
                <button type="button" disabled={busy || !permissions.edit || !changed} onClick={save} className="rounded-xl bg-slate-900 px-5 py-3 font-black text-white disabled:opacity-40">{busy ? <SpinnerGap className="inline animate-spin" /> : <FloppyDisk className="inline" />} حفظ مسودة</button>
                {draft?.status === "draft" && <button type="button" disabled={busy || !permissions.approve} onClick={approve} className="rounded-xl bg-amber-500 px-5 py-3 font-black text-white disabled:opacity-40"><CheckCircle className="inline" /> اعتماد</button>}
                {draft?.status === "approved" && <button type="button" disabled={busy || !permissions.publish} onClick={publish} className="rounded-xl bg-emerald-700 px-5 py-3 font-black text-white disabled:opacity-40"><PaperPlaneTilt className="inline" /> نشر إلى سلة</button>}
            </div>

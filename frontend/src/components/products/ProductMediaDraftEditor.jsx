...
import {
    approveProductMediaDraft, deleteProductMediaUpload, getProductMediaControl,
    publishProductMediaDraft, saveProductMediaDraft, uploadProductMediaFile,
...
function previewUrl(value) {
    ...
    try {
        ...
        if (parsed.pathname.includes("/api/products-v2/media-upload/file/")) return `${window.location.origin}${parsed.pathname}${parsed.search}`;
...
function cloneImages(rows = []) {
    return rows.map((row, index) => ({
        id: row.id || null, url: previewUrl(row.url), alt: row.alt || "",
        is_main: Boolean(row.is_main || row.main || index === 0), sort: index + 1,
        source: row.source || (row.id ? "salla" : "external_url"),
        upload_token: row.upload_token || null, filename: row.filename || null,
    }));
}
...
export default function ProductMediaDraftEditor({ productId, images = [], onPublished }) {
    ...
    async function load() {
        ...
        } catch (error) { toast.error(error?.response?.data?.detail?.message || "تعذر تحميل إدارة صور المنتج"); }
    }

    useEffect(() => { setEditing(false); setNewUrl(""); load(); }, [productId]); // eslint-disable-line react-hooks/exhaustive-deps
    ...
    function setMain(index) { setDraftImages(draftImages.map((row, rowIndex) => ({ ...row, is_main: rowIndex === index }))); }

    async function remove(index) {
        if (draftImages.length <= 1) return toast.error("يجب أن يبقى للمنتج صورة واحدة على الأقل");
        const row = draftImages[index];
        if (row.upload_token) { try { await deleteProductMediaUpload(productId, row.upload_token); } catch { /* TTL cleanup remains */ } }
        setDraftImages(normalizeMain(draftImages.filter((_, rowIndex) => rowIndex !== index)));
    }
    ...
    async function save() {
        ...
        } catch (error) { toast.error(error?.response?.data?.detail?.code || "تعذر حفظ مسودة الصور"); }
    ...
    async function publish() {
        ...
        try {
            ...
            await load(); setEditing(false); onPublished?.();

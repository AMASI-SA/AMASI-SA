...
export default function ProductControlCenterPanel({ productId, product, onPublished }) {
    ...
    const field = (key, label, input) => <label className="block text-xs font-black text-slate-600">{label}{input || <input value={form[key]} onChange={(event) => setForm((row) => ({ ...row, [key]: event.target.value }))} className="mt-1 w-full rounded-xl border border-slate-200 p-3 text-sm font-normal text-slate-950 outline-none focus:border-violet-500" />}</label>;
    ...
    return <section className="overflow-hidden rounded-2xl border border-violet-200 bg-white" data-testid="product-control-center">
        ...
        {tab === "edit" ? <div className="space-y-5 p-4">
            ...
            <div className="grid gap-4 md:grid-cols-2">
                ...
                {field("status", "حالة المنتج", <select value={form.status} onChange={(event) => setForm((row) => ({ ...row, status: event.target.value }))} className="mt-1 w-full rounded-xl border p-3 text-sm"><option value="active">نشط</option><option value="inactive">مخفي</option><option value="out_of_stock">نفد</option></select>)}
                {field("price", "السعر الأساسي", <input type="number" min="0" step="0.01" value={form.price} onChange={(event) => setForm((row) => ({ ...row, price: event.target.value }))} className="mt-1 w-full rounded-xl border p-3 text-sm" />)}
                {field("sale_price", "السعر المخفض", <input type="number" min="0" step="0.01" value={form.sale_price} onChange={(event) => setForm((row) => ({ ...row, sale_price: event.target.value }))} className="mt-1 w-full rounded-xl border p-3 text-sm" />)}
                {field("sale_starts_at", "تاريخ بداية التخفيض", <input type="date" value={form.sale_starts_at} onChange={(event) => setForm((row) => ({ ...row, sale_starts_at: event.target.value }))} className="mt-1 w-full rounded-xl border p-3 text-sm" />)}
                {field("sale_ends_at", "تاريخ نهاية التخفيض", <input type="date" value={form.sale_ends_at} onChange={(event) => setForm((row) => ({ ...row, sale_ends_at: event.target.value }))} className="mt-1 w-full rounded-xl border p-3 text-sm" />)}
            ...
            {field("short_description", "الوصف المختصر", <textarea rows={3} value={form.short_description} onChange={(event) => setForm((row) => ({ ...row, short_description: event.target.value }))} className="mt-1 w-full rounded-xl border p-3 text-sm" />)}
            {field("seo_description", "وصف SEO", <textarea rows={3} value={form.seo_description} onChange={(event) => setForm((row) => ({ ...row, seo_description: event.target.value }))} className="mt-1 w-full rounded-xl border p-3 text-sm" />)}
            <label className="block text-xs font-black text-slate-600">وصف المنتج<VisualHtmlEditor value={form.description} onChange={(description) => setForm((row) => ({ ...row, description }))} /></label>
            <label className="block text-xs font-black text-slate-600">سبب التعديل<input value={reason} onChange={(event) => setReason(event.target.value)} placeholder="مثال: تحسين الوصف بعد انخفاض الإضافة للسلة" className="mt-1 w-full rounded-xl border p-3 text-sm font-normal" /></label>

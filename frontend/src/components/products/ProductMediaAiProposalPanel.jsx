...
export default function ProductMediaAiProposalPanel({ productId, images = [] }) {
    ...
    async function load() {
        ...
        try {
            ...
            setSourceImageUrl((current) => (result.source_images || []).some((row) => row.url === current) ? current : "");
        } catch (error) { toast.error(error?.response?.data?.detail?.message || "تعذر تحميل طبقة ذكاء الصور"); }
        finally { setLoading(false); }
    }

    useEffect(() => { setState(null); setOperation(""); setSourceImageUrl(""); setPrompt(""); load(); }, [productId]); // eslint-disable-line react-hooks/exhaustive-deps

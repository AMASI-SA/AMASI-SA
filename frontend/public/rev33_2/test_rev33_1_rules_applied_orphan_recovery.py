...
def _mk_full_db(*, settings, rows, kill_switch_events=None):
    ...
    for cn in ("qoyod_customers", "qoyod_customer_mapping",
               ...
               # rev33.2 — `_get_api_client` calls `get_api_key`
               # which reads `qoyod_credentials`. Mock it so tests
               # exercising the canary-active path (which permits
               # live and thus requests real credentials) don't
               # blow up in the credential lookup.
               "qoyod_credentials"):
        c = MagicMock()
